...
enum ExpertLayout {
    ...
    private static func collectPairs(
        ...
        for key in selectedKeys {
            ...
            // The embedded MTP head keeps its split keys: the head loader
            // binds the canonical gate/up/down names, and a fused slab there
            // failed `adoptMTPHead`'s strict `verify: .all` update on the
            // ranked box. `bindMoE` serves the head through the split path
            // when the table carries no `mtp.*` entry.
            guard !modulePath(fromCheckpointStem: parsed.stem).hasPrefix("mtp.") else {
                continue
            }
