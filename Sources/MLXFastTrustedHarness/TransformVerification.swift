...
public enum TransformVerifier {
    public static func verify(_ options: TransformVerificationOptions) throws -> TransformVerificationReport {
        ...
        // REFUSE A TREE THAT DOES NOT DECLARE ITS NORM CONVENTION, and do it
        // before the multi-GB regeneration and byte compare so the refusal
        // names the real cause. The byte compare would also catch a stale
        // tree, but only as an opaque config.json mismatch; this check says
        // which key is missing and why it matters. The convention is fixed
        // when the norm modules are constructed, before any weight binds, so
        // no digest, shape or byte-count gate downstream can see it -- the
        // key in the file is the only thing that carries it to a consumer
        // that reads the tree. Fail closed: a missing key is a refusal, never
        // a default. Families with no such knob pass through untouched.
        try SwiftTransform.validateTransformedNormConvention(
            runtimeConfigPath: weightsDirectory.appendingPathComponent("config.json")
        )
