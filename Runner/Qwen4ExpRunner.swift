...
public final class TrackQwen4ExpRunner: Runner, @unchecked Sendable {
    ...
    public static func adopt(
        ...
        let head = try model.mtp.map {
            // MLXFAST-HEAD2: second rung down the head-bits ladder. HEAD3
            // (3 bits) sealed 1.9796 (-1.71%, our second-best) with the draft
            // mechanism working; 2-bit halves head traffic again at (likely)
            // lower acceptance. The acceptance/speed product decides; the
            // sealed metrics read it out. See adoptMTPHead.
            try Self.adoptMTPHead($0, configuration: model.configuration, headBits: 2)
        }
    ...
    public static func adoptMTPHead(
        ...
        // RE-FIRE 2: the fixed revision then died twice on the box's own
        // control leg over band before any candidate leg compared, so this
        // comment is the only delta over the proven-local revision):
