...
enum TrackResidualEpilogue {
    ...
    /// S=1 o_proj / GDN out_proj: simdgroups per threadgroup. 1 doubles
    /// independently schedulable threadgroups vs the tip's 2, keeping the
    /// 4-row `qmv_fast_reg` walk (RPS=2 already mismatched tokens).
    static let decodeSimdgroupsPerThreadgroup = 1
