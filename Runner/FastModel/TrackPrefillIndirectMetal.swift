...
struct BaseNAXFrag {
  ...
  /// and the output row stride is the kernel's `N`, which the launcher pins to
  /// 640 or 2560. The compiler will not merge the four stores itself for the
  ...
  METAL_FUNC static constexpr void store_vec4(
      const thread dtype_frag_t<T>& src, device U* dst, const int str_x) {
    const short2 sc = get_coord();
