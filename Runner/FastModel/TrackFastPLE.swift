...
enum TrackFastPLEKernels {
    ...
    ///   -> out [B,S,W] = residual + InT(gated + silu(conv))
    ...
    static let convSource = """
        ...
        InT ple = gated[o] + mlx_silu(static_cast<InT>(acc));
        out[o] = residual[o] + ple;
        """
    ...
    static func conv(
        full: MLXArray, convW: MLXArray, gated: MLXArray, residual: MLXArray, dilation: Int
    ) -> MLXArray {
