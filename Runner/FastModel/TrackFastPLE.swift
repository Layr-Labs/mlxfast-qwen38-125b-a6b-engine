...
enum TrackFastPLEKernels {
    ...
    // MARK: norm_key(key_proj(e)) * norm_query(stream), folded over the row

    /// keyFlat [B,S,W], stream [B,S,W], kscale [W], qscale [W], eps
    ///   -> dot [B,S,HC,1]
    /// Same launch shape as `prod`, but the product row is reduced in place
    /// with `row_reduce_looped`'s exact fold: a per-thread four-element chain
    /// in the activation dtype, `simd_sum`, one partial per simdgroup, and a
    /// second `simd_sum` over the 32-lane padded partials. MLX dispatches
    /// `row_reduce_looped` for this reduce whenever B*S*HC < 32 (S <= 7 here),
    /// so the result is bit-identical to `prod(...).sum(axis: -1)` on every
    /// window this kernel is called for. Saves the [B,S,W] product write, its
    /// readback, and one launch per window.
    /// grid (H/4, HC, B*S), threadgroup (H/4, 1, 1)
    static let prodDotSource = """
        ...
        const float kinv = metal::precise::rsqrt(kacc / (float)H + eps);
        const float qinv = metal::precise::rsqrt(qacc / (float)H + eps);
        // The looped row reduce: each thread's four products fold into a
        // running total in the activation dtype, then one simd_sum, then the
        // per-simdgroup partials fold through a second simd_sum. `InT(0) + dot`
        // is the non-row aggregate step (`total = op(total, row_total)` with a
        // single non-row reduction).
        InT dacc = InT(0);
        for (int i = 0; i < N_READS; ++i) {
            const uint d = lid * N_READS + i;
            const InT kn = static_cast<InT>(kx[i] * kinv) * kscale[hc * H + d];
    ...
    static func prodDot(
        ...
        let B = keyFlat.dim(0), S = keyFlat.dim(1), W = hcCount * hidden
        // The in-kernel fold reproduces `row_reduce_looped` only when MLX would
        // pick it (B*S*HC < 32) and when the threadgroup covers the row in one
        // four-element read per thread (hidden % 128 == 0, hidden/4 <= 1024).
        precondition(
            B * S * hcCount < 32 && hidden % 128 == 0 && hidden / 4 <= 1024
                && keyFlat.dim(2) == W)
