...
enum TrackFastPLEKernels {
    ...
    static let prodSource = """
        ...
        const uint kbase = row * KSTRIDE + hc * H;
        float kx[N_READS];
        float qx[N_READS];
        float kacc = 0.0f;
        float qacc = 0.0f;
    ...
    static let gatedSource = """
        ...
        float gx[N_READS];
        float acc = 0.0f;
            const InT v = sgm * value[row * VSTRIDE + (uint)VOFF + d];
            gated[base + d] = v;
    ...
    static func gated(
        ...
        hcCount: Int, hidden: Int, eps: Float, valueStride: Int? = nil, valueOffset: Int = 0
