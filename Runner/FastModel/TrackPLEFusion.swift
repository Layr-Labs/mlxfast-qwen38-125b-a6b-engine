...
enum TrackPLEFusion {
    ...
    // The fused prepare runs all group norms, dot, sigmoid, gate, the 4-tap
    // chronological convolution, and adds the residual directly into `added`.
    static let fusedPrepareSource = """
        // MLXFAST-PLEFUSE2: all three group norms, dot, gate, and concat.
        constexpr uint H = 2560;
        constexpr uint W = 4 * H;
        const uint hc = threadgroup_position_in_grid.y;
    ...
    static func forward(
        ...
        guard let res = forwardProjected(p, key: key, value: value, stream: stream, convState: convState, eps: eps, fusedResidual: false) else {
            return nil
        }
        return (res.full, res.output)
    }
}
private let gauntletRedraw_d0f22bb7_20260923T103531Z: Int = 0
