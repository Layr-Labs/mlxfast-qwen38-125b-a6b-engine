...
// This retains the RMS/reduction lane layout; the dilated convolution now
// runs channel-per-thread like track_ple_conv. Token tolerance applies.
import Foundation
import MLX
...
enum TrackPLEFusion {
    ...
    static let convolutionSource = """
        // MLXFAST-PLEFUSE3: one channel per thread, the same layout
        // track_ple_conv already runs for S >= 2. The implicit-GEMM staging
        // is gone: no simdgroup broadcast, no lane-0 owner, no padded
        // matrix work, no threadgroup storage. The fp32 products and their
        // chronological additions are identical to the staged version's:
        // acc = tap0, then taps 1,2,3 in order, one rounding at the end.
