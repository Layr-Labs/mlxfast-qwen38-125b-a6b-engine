...
enum TrackPLEFusion {
    ...
    static let convolutionSource = """
        ...
        // unchanged — the same four FP32 products accumulated in tap order
        // 0,1,2,3, the same silu, the same gated add — so the output is
        // bit-identical; only which thread owns a channel changes.
        ...
        float acc = float(full[c]) * float(w[0]);
        acc += float(full[3 * W + c]) * float(w[1]);
        acc += float(full[6 * W + c]) * float(w[2]);
        acc += float(full[9 * W + c]) * float(w[3]);
