...
enum TrackPLEFusion {
    ...
    static let convolutionSource = """
        // MLXFAST-PLEFUSE3: one channel per lane. The fused kernel replaces
        // the dilated implicit-GEMM dispatch (group=(32,1,4), grid
        // (1,1,W), channel=group.z) whose small-channel loaders assigned
        // taps 0..3 to SIMD 0 lanes 0..3 and let lane 0 alone store: 10240
        // threadgroups of 128 threads each, 124 of them dead. Here each
        // lane owns one channel and runs the same arithmetic serially:
        // four FP32 products accumulated in chronological tap order 0,1,2,3,
        // one rounding into InT, silu, and the residual add. The per-channel
        // rounding sequence is identical to the broadcast version, which was
        // itself identical to the reference conv; only the thread-to-channel
        // map changes. No threadgroup storage, no barriers.
        constexpr uint W = 10240;
        const uint c = thread_position_in_grid.x;
        if (c >= W) { return; }
        // full is [1, 10, W] row-major: tap t sits at row t*3, channel c.
    ...
    static func forward(
        ...
        // 10240 channels, one per lane: 80 threadgroups of 128 threads.
        let output = convolutionKernel(
            [r[1], p.convW, r[0]], template: [("InT", stream.dtype)],
            grid: (10240, 1, 1), threadGroup: (128, 1, 1),
