...
// Scratch: ONE reused float[32] (128 B) in prepare, no second launch at all:
// the dilated convolution is folded into prepare because its taps are the
// carried state rows 0/3/6 and the normed row the same threadgroup just
// produced. Token tolerance, not bit equality, applies.
import Foundation
import MLX
...
enum TrackPLEFusion {
    ...
    static let prepareSource = """
        ...
        // The dilated convolution, folded into this launch: its taps are the
        // carried state rows 0, 3, 6 and the normed row this threadgroup just
        // produced. The same four float products in chronological tap order,
        // one InT rounding, silu, and the residual add the separate launch
        // computed -- with the gated row kept in registers instead of a
        // materialized [1, 1, W] output.
        for (uint i = 0; i < 4; ++i) {
            const uint c = base + i;
            float accv = float(convState[0 * W + c]) * float(convw[c * 4 + 0]);
            accv += float(convState[3 * W + c]) * float(convw[c * 4 + 1]);
            accv += float(convState[6 * W + c]) * float(convw[c * 4 + 2]);
            accv += float(nrm[i]) * float(convw[c * 4 + 3]);
            out[c] = g[i] + mlx_silu(InT(accv));
        }
        """
