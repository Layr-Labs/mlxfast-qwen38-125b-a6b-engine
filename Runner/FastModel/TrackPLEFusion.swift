...
// The cap is the 32-row boundary of MLX's row reduce: the dot runs
// row_reduce_looped's fold, which the reference keeps through 4S < 32.
import Foundation
import MLX
...
enum TrackPLEFusion {
    ...
    static let prepareSource = """
        ...
        for (uint i = 0; i < 4; ++i) {
            ...
            k = k * keyScale[hc * H + d + i];
            InT q = InT(float(query[base + i]) * iq);
            q = q * queryScale[hc * H + d + i];
            InT product = k * q;
            dot = product + dot;
        }
        dot = InT(0) + dot;
        ...
        for (uint i = 0; i < 4; ++i) {
            ...
            full[9 * W + base + i] = n * convScale[hc * H + d + i];
        }
        // Same [old nine rows, new row] layout as concatenated. Only row 0's
        // threadgroups stage the carried state; the normed rows land per row.
        if (row == 0) {
            for (uint t = 0; t < 9; ++t) {
                for (uint i = 0; i < 4; ++i) {
                    full[t * W + hc * H + d + i] = convState[t * W + hc * H + d + i];
                }
            }
        }
        """
    ...
    static let convolutionSource = """
        // MLXFAST-PLEFUSE2: preserve the dilated implicit-GEMM dispatch:
        // group=(32,1,4), grid of groups=(1,S,W), channel=group.z, row=group.y.
        ...
        float product = 0.0f;
        if (lane < 4) {
            product = float(full[(t + lane * 3) * W + c]) * float(weight[c * 4 + lane]);
        }
        ...
        if (lane == 0) {
            ...
            out[t * W + c] = gated[t * W + c] + activated;
        }
