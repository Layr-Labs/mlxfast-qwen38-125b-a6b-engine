...
enum TrackPLEFusion {
    ...
    static let prepareSource = """
        ...
        const uint cb = hc * H + d;
        const uint vb = row * H + d;
        ...
        for (uint i = 0; i < 4; ++i) {
            ...
            full[(9 + row) * W + cb + i] = n * convScale[cb + i];
        }
        ...
        if (row == 0) {
            for (uint t = 0; t < 9; ++t) {
                for (uint i = 0; i < 4; ++i) {
                    full[t * W + cb + i] = convState[t * W + cb + i];
                }
            }
        }
        """
    ...
    static func supports(_ p: TrackPLE, stream: MLXArray, hidden: Int, hcCount: Int) -> Bool {
        ...
        hidden == 2560 && hcCount == 4 && stream.dim(0) == 1
            && stream.dim(1) >= 1 && stream.dim(1) <= 7 && stream.dim(2) == 10240
