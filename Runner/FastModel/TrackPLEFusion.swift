...
enum TrackPLEFusion {
    ...
    static let convolutionSource = """
        // MLXFAST-PLEFUSE2: one thread per channel. The reference S=1 dilated
        // implicit GEMM staged taps 0..3 on SIMD lanes 0..3 and let lane 0
        // store, leaving 124 of 128 lanes idle; its arithmetic is four FP32
        // products summed in chronological tap order with one rounding, which
        // a single thread performs identically -- same operands, same order,
        // same InT rounding before silu and the residual add.
