...
enum TrackPrefillMixFuse {
    static let fuseHeader = #"""
template <typename U, int N, int bits, typename W>
inline void mix_fuse_dequantize(const device uint8_t* w, U scale, U bias, W w_local) {
  static_assert(
...
struct MixFuseBlockLoader<
  ...
  void load_unsafe() const thread {
    ...
    for (int i = 0; i < n_reads; i++) {
      mix_fuse_dequantize<T, pack_factor, bits>(
...
template <
    ...
    const int WN = 2,
    const int DBG = 0,
    const bool HAS_INJ = false>
METAL_FUNC void track_mix_fused(
  ...
  using loader_w_t = MixFuseBlockLoader<
      T,
      BN,
      BK,
      BK_padded,
  ...
  scales += y_col * K_g;
  biases += y_col * K_g;
  // MLXFAST-MIXFUSE: the epilogue writes the [M, HH] mixer output with
  // absolute indices, so `y` keeps its base.
  ...
  dispatch_bool(!is_unaligned_sm, [&](auto kAlignedM) {
    dispatch_bool(aligned_N || !is_unaligned_bn, [&](auto kAlignedN) {
      auto run = [&](auto kPrefetch) {
        ...
        // MLXFAST-MIXFUSE: round exactly as the device store would, stage the
        // tile in the weight buffer the K loop has finished with, then fold the
        // four hyper-connection streams here instead of writing [S, HC*H] out
        // and reading it straight back.
        constexpr short WS_PAD = BK_padded;
        for (short e = 0; e < Dtile.kElemsPerTile; ++e) {
          Dtile.elems()[e] = static_cast<AccumType>(static_cast<T>(Dtile.elems()[e]));
        }
        Dtile.template store<T, WS_PAD, 1>(Ws + tm * WS_PAD + tn);
        threadgroup_barrier(mem_flags::mem_threadgroup);
        // Packed rows: eight consecutive columns are (d, d+1) x four streams,
        // stream s at offset s*2 + c. A BN-wide tile therefore holds BN/4
        // complete d columns.
        constexpr short DPT = BN / 4;
        const short dBase = short(tid.x) * DPT;
        const short nthreads = WM * WN * SIMD_SIZE;
        for (short o = short(lid); o < BM * DPT; o += nthreads) {
          const short t = o / DPT;
          const short u = o - t * DPT;
          const short g = u >> 1, c = u & 1;
          const int token = y_row + t;
          if (token >= M) { continue; }
          const int d = int(dBase) + 2 * int(g) + int(c);
          if (DBG == 1) {
            // Diagnostic: pass the staged stream-0 value straight through, so
            // the staging and the column map can be checked without the fold.
            y[(size_t)token * (size_t)HH + (size_t)d] = Ws[t * WS_PAD + g * 8 + c];
          } else {
            T acc = T(0);
            for (short sIdx = 0; sIdx < 4; ++sIdx) {
              const T wv = Ws[t * WS_PAD + g * 8 + sIdx * 2 + c];
              const T sg = mlx_sigmoid(wv);
              const T p = sg * normed[(size_t)token * (size_t)(4 * HH) + (size_t)sIdx * (size_t)HH + (size_t)d];
              acc = acc + p;
            }
            y[(size_t)token * (size_t)HH + (size_t)d] = acc;
          }
        }
        if (HAS_INJ && tid.x == 0) {
          for (short o = short(lid); o < BM * HC; o += nthreads) {
            const short t = o / HC;
            const short e = o - t * HC;
            const int token = y_row + t;
            if (token >= M) { continue; }
            const T xv = inj[(size_t)token * (size_t)LW + (size_t)(LW - HC) + (size_t)e];
            inject[(size_t)token * (size_t)HC + (size_t)e] = T(2) * mlx_sigmoid(xv);
          }
        }
      };
      if constexpr (
