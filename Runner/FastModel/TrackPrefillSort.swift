...
enum TrackPrefillSort {
    ...
    static let scatterSource = #"""
        ...
        // the same adds in the same order; only which array is read and which is
        // written alternates, and both branches are uniform across the
        // threadgroup. The final array depends on the parity of log2(E), so an
        // odd round count is copied back into sA once.
        uint r = 0;
