...
template <
    ...
    int WN,
    bool transpose>
METAL_FUNC void track_prefill_gate_up(
    const device T* x,
    const device uint32_t* w,
    const device T* scales,
    const device T* biases,
    const device uint32_t* wu,
    const device T* scales_u,
    const device T* biases_u,
    const device uint32_t* indices,
    device T* y,
    int M,
    int N,
  ...
  auto ul = (const device uint8_t*)wu + size_t(y_col) * K_w;
  scales_u += size_t(y_col) * K_g;
  biases_u += size_t(y_col) * K_g;
  ...
  while (n < tgp_bm) {
    ...
    thread loader_w_t loader_u(
        ul + index * stride_w,
        scales_u + index * stride_s,
        biases_u + index * stride_s,
        K,
        Ws_u,
        simd_group_id,
        simd_lane_id);
    ...
    dispatch_bool(tile_m == BM, [&](auto kAlignedM) {
      ...
      for (int k = 0; k < K_it; k++) {
        ...
        for (int kk1 = 0; kk1 < BK; kk1 += SK) {
          if (sg_active) {
            ...
            Btile.template load<T, BK_padded, 1>(Ws_u + tn * BK_padded + kk1);
            tile_matmad_nax(
                Utile,
                Atile,
                metal::bool_constant<false>{},
      ...
      if (sg_active) {
        ...
        for (short e = 0; e < Dtile.kElemsPerTile; ++e) {
          const T g = static_cast<T>(Dtile.elems()[e]);
          const T u = static_cast<T>(Utile.elems()[e]);
          const T activated = mlx_silu(g) * u;
