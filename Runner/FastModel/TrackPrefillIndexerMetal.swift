...
extension TrackPrefillIndexer {
    static let denseHeader = #"""
template <typename U, int N, int bits, typename W>
inline void track_indexer_dequantize(const device uint8_t* w, U scale, U bias, W w_local) {
  static_assert(
...
struct TrackIndexerBlockLoader;
template <
    typename T,
    short BROWS,
    short BCOLS,
...
struct TrackIndexerBlockLoader<
  ...
  void load_unsafe() const thread {
    ...
    for (int i = 0; i < n_reads; i++) {
      track_indexer_dequantize<T, pack_factor, bits>(
...
template <
    ...
    const int WM = 2,
    const int WN = 2>
METAL_FUNC void track_indexer_dense(
  ...
  using loader_w_t = TrackIndexerBlockLoader<
      T,
      BN,
      BK,
      BK_padded,
