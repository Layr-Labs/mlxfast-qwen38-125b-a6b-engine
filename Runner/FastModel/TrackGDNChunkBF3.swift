...
/// Each NAX panel starts at zero; FP32 TwoSum carries the panel-addition error.
...
enum TrackGDNChunkBF3 {
    ...
    static let helpers = #"""
    // Error-free TwoSum for the outer FP32 panel addition, followed by an
    // FP32 fold of the residuals. This does not make NAX's internal dot exact.
    // In particular, do not seed NAX with the previous accumulator: a small
    // component can otherwise disappear before two later large panels cancel.
    METAL_FUNC float2 gdn_panel_add(float sum, float error, float term) {
      #pragma clang fp reassociate(off)
      #pragma clang fp contract(off)
      const float next = sum + term;
      const float recovered = next - sum;
      const float residual = (sum - (next - recovered)) + (term - recovered);
      return float2(next, error + residual);
    }
    ...
    // by its BF16 components. The panel errors survive later cancellation.
    template <typename T>
    METAL_FUNC void gdn_bf3_project(
      ...
      auto panel = op.template get_destination_cooperative_tensor<AT, BT, float>();
      NAXTile<float, 1, 2> z_error, out_error;
      z.clear(); out.clear(); z_error.clear(); out_error.clear();
      STEEL_PRAGMA_UNROLL
      for (int part = 0; part < 3; ++part) {
        ...
        for (short step = 0; step < 8; ++step) {
          ...
          STEEL_PRAGMA_UNROLL
          for (short e = 0; e < 16; ++e) { panel[e] = 0.0f; }
          op.run(a, b, panel);
          STEEL_PRAGMA_UNROLL
          for (short f = 0; f < 2; ++f) {
            STEEL_PRAGMA_UNROLL
            for (short e = 0; e < 8; ++e) {
              const float2 pair = gdn_panel_add(z.val_frags[f][e], z_error.val_frags[f][e], panel[f * 8 + e]);
              z.val_frags[f][e] = pair.x; z_error.val_frags[f][e] = pair.y;
            }
          }
          STEEL_PRAGMA_UNROLL
          for (short e = 0; e < 8; ++e) {
          ...
          for (short f = 0; f < 2; ++f) {
            STEEL_PRAGMA_UNROLL
            for (short e = 0; e < 8; ++e) {
              const float2 pair = gdn_panel_add(out.val_frags[f][e], out_error.val_frags[f][e], panel[f * 8 + e]);
              out.val_frags[f][e] = pair.x; out_error.val_frags[f][e] = pair.y;
            }
          }
        }
      }
      ...
      for (short f = 0; f < 2; ++f) {
        STEEL_PRAGMA_UNROLL
        for (short e = 0; e < 8; ++e) {
          z.val_frags[f][e] += z_error.val_frags[f][e];
          out.val_frags[f][e] += out_error.val_frags[f][e];
        }
      }
    }
    ...
    METAL_FUNC void gdn_bf3_update(
      ...
      for (short block = 0; block < 4; ++block) {
        auto panel = op.template get_destination_cooperative_tensor<AT, BT, float>();
        float sums[16], errors[16];
        STEEL_PRAGMA_UNROLL
        for (short f = 0; f < 2; ++f) {
          STEEL_PRAGMA_UNROLL
          for (short e = 0; e < 8; ++e) {
            sums[f * 8 + e] = decay * state.val_frags[2 * block + f][e];
            errors[f * 8 + e] = 0.0f;
          }
        }
        STEEL_PRAGMA_UNROLL
        for (int part = 0; part < 3; ++part) {
          ...
          for (short step = 0; step < 2; ++step) {
            ...
            for (short e = 0; e < 16; ++e) {
              const float2 pair = gdn_panel_add(sums[e], errors[e], panel[e]);
              sums[e] = pair.x; errors[e] = pair.y;
            }
          }
        }
        STEEL_PRAGMA_UNROLL
        for (short f = 0; f < 2; ++f) {
          STEEL_PRAGMA_UNROLL
          for (short e = 0; e < 8; ++e) {
            state.val_frags[2 * block + f][e] = sums[f * 8 + e] + errors[f * 8 + e];
          }
        }
      }
    }
    ...
    static let gramSource = #"""
      ...
      float sums[16], errors[16];
      STEEL_PRAGMA_UNROLL
      for (short e = 0; e < 16; ++e) { sums[e] = 0.0f; errors[e] = 0.0f; }
      ...
      for (short step = 0; step < 8; ++step) {
        ...
        STEEL_PRAGMA_UNROLL
        for (short e = 0; e < 16; ++e) {
          const float2 pair = gdn_panel_add(sums[e], errors[e], c[e]);
          sums[e] = pair.x; errors[e] = pair.y;
        }
      }
      ...
      for (short f = 0; f < 2; ++f) {
        ...
        for (short e = 0; e < 8; ++e) {
          ...
          destination[((sg % 2) * 16 + coord.y) * 32 + f * 16 + coord.x] = sums[f * 8 + e] + errors[f * 8 + e];
        }
      }
    """#
