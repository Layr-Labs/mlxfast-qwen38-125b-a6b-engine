...
final class TrackPLESSMStaging: @unchecked Sendable {
    ...
    func writeDecode<C: Collection>(_ values: C) -> MLXArray where C.Element: BinaryInteger {
        ...
        let i = decodeFlip
        decodeFlip ^= 1
        // Re-eval then take the pointer: a noCopy pointer from init is
        // invalidated when later GPU evals recycle the MLX buffer.
        eval(decode[i])
        let dest = Self.int32Pointer(decode[i])
        var j = 0
        for value in values {
            dest[j] = Int32(value)
            j += 1
    ...
    func writeCapture(history: [Int64], newCount: Int) -> MLXArray {
        ...
        captureFlip ^= 1
        eval(capture[i])
        let dest = Self.int32Pointer(capture[i])
        for s in 0 ..< newCount {
