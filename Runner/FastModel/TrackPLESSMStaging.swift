...
final class TrackPLESSMStaging: @unchecked Sendable {
    ...
    func writeDecode<C: Collection>(_ values: C) -> MLXArray where C.Element: BinaryInteger {
        ...
        // Ranked free-run mismatched at step 2 for: escaped 1-row noCopy
        // pointers, MLX subscript-set into reused buffers, S=1 skip, and
        // asStrided 1-row views of 8-row storage. A fresh contiguous
        // [1, ctx] array is the live buffer the next forward reads.
        let ints = values.map { Int32($0) }
        let out = MLXArray(ints, [1, contextLength])
        eval(out)
        return out
    }
