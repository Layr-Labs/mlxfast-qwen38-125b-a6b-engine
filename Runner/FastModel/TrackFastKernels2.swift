...
extension TrackFastKernels {
    ...
    static let gatedRMSSource = """
        ...
        // One simdgroup per (row, head): the first simd_sum already produces
        // the row total on every lane. The reference staged it through
        // threadgroup memory and re-reduced [total, 0, ...] — the same value,
        // one barrier and one shared-memory round trip heavier.
        acc = simd_sum(acc);
