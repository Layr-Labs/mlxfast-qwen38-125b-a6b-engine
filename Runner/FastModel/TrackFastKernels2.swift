...
extension TrackFastKernels {
    ...
    static let gatedRMSSource = """
        ...
        const uint row = thread_position_in_grid.z;
        // MLXFAST-GATEDRMS-SINGLE: one SIMD group per threadgroup, so the
        // first simd_sum already holds the row total on every lane. The old
        // second reduction summed [total,0,...,0] back to total.
