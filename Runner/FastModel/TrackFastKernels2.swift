...
extension TrackFastKernels {
    ...
    static func attnPrep(
        ...
        // of `outputDTypes`. There is no early return between them, so binding
        // it once turns four reads into one. Host-side only: the same `DType`
        // fills the same template slot and describes the same three outputs,
        // so the kernel, every template constant, the grid, the threadgroup
        // shape and every byte moved are unchanged.
