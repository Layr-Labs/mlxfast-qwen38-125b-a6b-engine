...
extension TrackFastKernels {
    ...
    static let gatedRMSSource = """
        ...
        const uint row = thread_position_in_grid.z;
        const uint lane = thread_index_in_simdgroup;
        // One simdgroup per threadgroup: `simd_sum` below already gives every
        // lane the row total, so there is no cross-simdgroup partial to stage
        // through shared memory (unlike `injectNorm`/`attnPrep`, which fold
        // several simdgroups through `local_sums`).
