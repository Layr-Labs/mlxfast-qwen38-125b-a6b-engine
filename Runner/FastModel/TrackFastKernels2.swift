...
extension TrackFastKernels {
    ...
    /// Simdgroups per (row, stream) in the wide inject+norm launch. Each slice's
    /// partial is computed by the same lanes and folded with the same simd_sum
    /// whatever this is, so the sums are bit-identical; only the number of
    /// slices a thread owns changes (32/SG), and with it the launch's
    /// parallelism: at the prefill shape 4 leaves 4,096 threadgroups of 128
    /// threads and 8 leaves the same count of 256-thread ones.
