...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func pleForward(
                ...
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            ...
            // norm_key * norm_query, reduced over the last axis. MLX picks
            // `row_reduce_looped` iff B*S*hcCount < 32 (S <= 7); the fused
            // kernel reproduces that fold exactly and skips the product
            // buffer. Wider windows keep the generic reduce.
            let dot: MLXArray
            if B * S * hcCount < 32 {
