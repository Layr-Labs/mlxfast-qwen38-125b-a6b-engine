...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func pleForward(
        ...
        // MLXFAST-PLEFUSE2: two unchanged GEMVs + prepare + convolution for
        // S <= 7 (the row_reduce_looped window); larger windows take the
        // three-launch PLE block below.
        let full: MLXArray
        let output: MLXArray
        if S <= 7, TrackPLEFusion.supports(p, stream: stream, hidden: hidden, hcCount: hcCount),
