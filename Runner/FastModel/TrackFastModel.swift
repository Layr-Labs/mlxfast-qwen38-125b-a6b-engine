...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func hcMix(_ hc: TrackHC, normed: MLXArray) -> (MLXArray, MLXArray) {
        let lo = hc.downInject.apply(normed)  // [B,S,lowrank(+hc)]
        let act = TrackFastKernels.siluHead(lo: lo, width: hc.lowrank)
        let w = hc.up.apply(act)  // [B,S,W], pre-sigmoid
        return TrackFastKernels.hcMix(
            w: w, normed: normed, lo: lo, hcCount: hcCount, hidden: hidden,
    ...
    func fastStreams(
        ...
        for layer in layers {
            ...
            tile = false
            var (input, injectW) = hcMix(layer.attnHC, normed: normed)
            ...
            (input, injectW) = hcMix(layer.mlpHC, normed: normed)
