    ...
    private func pleForward(
                ...
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            ...
            if capture {
                ...
                // MLXFAST-PLECONVSPLIT: the conv reads the carried state and the
                // normed rows in place and emits the next conv state itself, so
                // the [n+S, W] `concatenated` copy is skipped on every decode
                // step. `full` below is only read for its last `stateLength`
                // rows, which are exactly `nextState`.
                let split = TrackFastPLEKernels.convSplit(
                    convState: convState, normed: gn.normed, convW: p.convW2,
                    gated: gated, dilation: p.dilation)
                full = split.nextState
                output = split.out
            }
        }
        do {
