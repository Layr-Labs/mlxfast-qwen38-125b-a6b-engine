...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func pleForward(
                ...
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            ...
            let gated: MLXArray
            let normedRows: MLXArray
            if B * S * hcCount < 32 {
                // MLXFAST-PLEGATEMERGE: one launch for the two norms, the dot,
                // the gate chain, and norm_conv. The in-kernel dot reproduces
                // row_reduce_looped's fold, which is the kernel MLX picks while
                // B*S*HC < 32; larger windows keep the three-launch chain.
                let mg = TrackFastPLEKernels.mergedGate(
                    keyFlat: keyFlat, stream: stream, value: value,
                    kScale: p.normKeyScale, qScale: p.normQueryScale,
                    cScale: p.normConvScale, hcCount: hcCount, hidden: hidden, eps: eps)
                gated = mg.gated
                normedRows = mg.normed
            } else {
                ...
                let gn = TrackFastPLEKernels.gated(
                    ...
                    hcCount: hcCount, hidden: hidden, eps: eps)
                gated = gn.gated
                normedRows = gn.normed
            }
            full = concatenated([convState, normedRows], axis: 1)  // [1, n+S, wide]
