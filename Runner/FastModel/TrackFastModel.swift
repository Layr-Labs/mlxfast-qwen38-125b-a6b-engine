...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    public init(base: Qwen4ExpModel) {
        ...
        // Build the wide replica's pipeline for every window the decode path
        // can hand it, here at load, so the first forward that is measured does
        // not also compile it. One dispatch per window on zeroed activations;
        // the results are discarded and nothing else is touched.
        if let g = built.compactMap({ $0.gdn }).first, let fusedProj = g.proj.fused,
            case .quant(let pq) = fusedProj, let pb = pq.biases,
            pq.rows >= TrackQuantWeight.wideReplicaMinRows, pq.bits == 4,
            pq.mode == .affine, pq.rows % 8 == 0
        {
            let K = pq.weight.dim(1) * 32 / pq.bits
            for window in 2 ... 8 {
                let probe = MLXArray.zeros([window, K], dtype: pq.scales.dtype)
                let warmed = TrackFastMoEKernels.wideCheck(
                    w: pq.weight, scales: pq.scales, biases: pb, x: probe,
                    groupSize: pq.groupSize, bits: pq.bits)
                eval(warmed)
            }
        }
        super.init()
