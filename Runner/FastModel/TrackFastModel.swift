...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func hcMix(_ hc: TrackHC, normed: MLXArray, tag: String = "") -> (MLXArray, MLXArray) {
        ...
        // Windows of two to eight tokens only, the same bound `TrackMultiProj`
        // applies: row concatenation is exact on the GEMV path, but a wide
        // input dispatches the split-K GEMM, which chooses its split from N,
        // so a fused N would not stay exact with the parts run separately.
        // One-token windows keep the fused mixer-head path they already had.
        if (2 ... 8).contains(normed.dim(1)), let fused = hc.downInject, hc.hasInject {
