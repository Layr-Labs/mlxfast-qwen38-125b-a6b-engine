...
struct TrackMoE {
    ...
    /// Device counter for the fused router + route launch (one per layer, zero at rest).
    let routeTicket: MLXArray
}
...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    /// Fused router + route launch on the one-token path (env `TRACK_FUSED_ROUTE=0` keeps the two launches).
    nonisolated(unsafe) static var fusedRouteEnabled: Bool =
        ProcessInfo.processInfo.environment["TRACK_FUSED_ROUTE"] != "0"
    ...
    static func bindMoE(_ m: Module, cfg: Qwen4ExpTextConfiguration) -> TrackMoE {
        ...
        // Pair index of one expert stack, built and verified from its loaded
        // bf16 metadata; on success the bf16 arrays (15 GB over the model) are
        // replaced, in the module and in this binding, by lazy reconstructions
        // from the index and table, so the index is memory-neutral (-7.5 GB net).
        func indexedExpert(_ key: String) -> (stack: (w: MLXArray, s: MLXArray, b: MLXArray), lut: TrackAffineLUT?) {
            let e = expert(key)
            guard TrackAffineLUT.enabled, TrackAffineLUT.expertsEnabled, qdown.bits == 4, qdown.groupSize == 32,
                let lut = TrackAffineLUT.build(scales: e.s, biases: e.b)
            else { return (e, nil) }
            guard TrackAffineLUT.releaseBF16 else { return (e, lut) }
            let s = lut.lazyScales().reshaped(e.s.shape), b = lut.lazyBiases().reshaped(e.b.shape)
            switchMLP.trackChild(key).update(parameters: ModuleParameters.unflattened(["scales": s, "biases": b]))
            return ((w: e.w, s: s, b: b), lut)
        }
        let ig = indexedExpert("gate_proj"), iu = indexedExpert("up_proj"), id = indexedExpert("down_proj")
        ...
        return TrackMoE(
            ...
            expertGate: ig.stack, expertUp: iu.stack, expertDown: id.stack,
            expertMeta: {
                guard let g = ig.lut, let u = iu.lut, let d = id.lut else { return nil }
                return (gate: g, up: u, down: d)
            }(),
            ...
            routeTicket: {
                // 16 words, not 1: MLX passes arrays shorter than 8 elements in the
                // constant address space, which the kernel could not update.
                let t = MLXArray.zeros([16], dtype: .uint32)
                eval(t)
                return t
            }())
    }
    ...
    /// One bf16 element, evaluated once: the replay input for metadata slots the
    /// indexed kernels do not read.
    nonisolated(unsafe) static let replayPlaceholder: MLXArray = {
        let a = MLXArray.zeros([1], dtype: .bfloat16)
        eval(a)
        return a
    }()
    ...
    static func moeForwardShared(
        ...
        // One token with a quantized gate: router GEMV + top-k/softmax + gate in
        // one launch (last-arriver ticket); the results feed the decode block below.
        var fusedRoute: (idx: MLXArray, w: MLXArray, gate: MLXArray)? = nil
            ...
            m.routerW16.dim(0) % 16 == 0, x.dim(2) < 16 * m.routerW16.dim(0), m.routerW16.dim(0) < 4096
        {
            ...
            if let gq = gateQ, Self.fusedRouteEnabled, gq.rows == 1, gq.bits == 4,
                m.routerW16.dim(0) % 4 == 0, x.dim(2) % 256 == 0, m.topK <= 32,
                StreamOrDevice.default.stream === Stream.gpu
            {
                let r = TrackFastMoEKernels.routerRoute(
                    x: xf, w: m.routerW16, xb: x.reshaped(x.dim(2)), sharedGate: gq, topK: m.topK,
                    ticket: m.routeTicket)
                logits = r.logits.reshaped(1, 1, -1)
                fusedRoute = (r.idx.reshaped(1, m.topK), r.w.reshaped(1, m.topK), r.gate)
            } else {
            ...
            case .quant(let dq) = m.sharedDown, guq.biases != nil, dq.biases != nil
        {
            ...
            if let replay, StreamOrDevice.default.stream === Stream.gpu {
                // With the expert index the replay's kernels read the index and
                // table; the bf16 slots then carry a one-element placeholder so a
                // lazily reconstructed array is never materialized by the call.
                let lutOn = Self.moeReplayUsesLut(m)
                let ph = Self.replayPlaceholder
                return replay([
                    x2, flatIdx, weights.reshaped(S * K), gate, xrow,
                    m.expertGate.w, lutOn ? ph : m.expertGate.s, lutOn ? ph : m.expertGate.b,
                    m.expertUp.w, lutOn ? ph : m.expertUp.s, lutOn ? ph : m.expertUp.b,
                    guq.weight, guq.scales, guq.biases!,
                    m.expertDown.w, lutOn ? ph : m.expertDown.s, lutOn ? ph : m.expertDown.b,
...
// MLXFAST-TAG-ropetabr15 (20260923-052911-15): build tag.
