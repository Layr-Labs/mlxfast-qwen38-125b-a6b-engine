    ...
    private func pleForward(
                ...
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            ...
            if capture {
                ...
                // MLXFAST-PLECONVSTATE: the conv reads convState and normed
                // directly and writes the new carried state in the same
                // launch, so the [1, n+S, wide] concat is never materialized.
                // `full` becomes the new state itself; the stage() slice below
                // is then an identity view of an already-materialized array.
                let cs = TrackFastPLEKernels.convState(
