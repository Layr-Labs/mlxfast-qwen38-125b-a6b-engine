    ...
    private func pleForward(
        ...
        do {
            if capture {
                ...
                let ssm: MLXArray
                // S=1 scored decode: use the device id slice. writeDecode's
                // MLX subscript-set was ~12% decode vs the pointer path and
                // the stored noCopy pointer SIGSEGVs after GPU evals.
                if let h = hostHistory, S != 1 {
