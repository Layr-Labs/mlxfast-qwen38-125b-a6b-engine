...
struct TrackMoE {
    ...
    var indexedGateUpMetadata: [MLXArray] = []
}
...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    static func bindMoE(_ m: Module, cfg: Qwen4ExpTextConfiguration) -> TrackMoE {
        ...
        if StreamOrDevice.default.stream === Stream.gpu,
            ...
            sharedQ.mode == .affine, sharedQ.groupSize == 32, sharedQ.bits == 4,
            sharedQ.scales.shape == [1280, 80], let sharedB = sharedQ.biases,
            let gateCache = TrackIndexedAffine.cache(scales: result.expertGate.s, biases: result.expertGate.b),
            let upCache = TrackIndexedAffine.cache(scales: result.expertUp.s, biases: result.expertUp.b),
            let sharedCache = TrackIndexedAffine.cache(scales: sharedQ.scales, biases: sharedB)
        {
            result.indexedGateUpMetadata = gateCache + upCache + sharedCache
            eval(result.indexedGateUpMetadata)
        }
        return result
    }
    ...
    static func makeMoEPairReplay(_ m: TrackMoE) -> (@Sendable ([MLXArray]) -> [MLXArray])? {
        ...
        return compile(shapeless: false) {
            [groupSize = m.expertGroupSize, bits = m.expertBits, topK = m.topK,
             ...
             hasIndexedMetadata = !m.indexedGateUpMetadata.isEmpty] inputs in
            ...
            let act = TrackFastMoEKernels.gateUpAct(
                ...
                metadata: hasIndexedMetadata ? Array(inputs[20..<26]) : [])
    ...
    static func moeForwardShared(
            ...
            case .quant(let dq) = m.sharedDown, guq.biases != nil, dq.biases != nil
        {
            ...
            if let replay, StreamOrDevice.default.stream === Stream.gpu {
                ...
                ] + m.indexedGateUpMetadata)[0].reshaped(1, S, H)
            }
            let act = TrackFastMoEKernels.gateUpAct(
                ...
                metadata: m.indexedGateUpMetadata)
            return TrackFastMoEKernels.downCombine(
