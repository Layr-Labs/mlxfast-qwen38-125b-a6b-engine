...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func pleForward(
        ...
        if let prefetched = plePrefetch.awaitResult(offset: offset, count: S) {
            // MLXFAST-PLEPF: the helper ran the same ids readback, context
            // resolution, hash and row gather from window entry; only the
            // mirror bookkeeping stays here so it is ordered after the await.
    ...
    func fastStreams(
        ...
        // MLXFAST-PLEPF: kick the PLE host block before any of this window's
        // graph is built, so its ids readback overlaps the previous window's
        // GPU tail and the hash + row gather overlap the construction below.
