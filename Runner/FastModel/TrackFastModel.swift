...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
        ...
        -> TrackLayer
    {
        ...
        return TrackLayer(
            ...
            gdnReplay: gdn.flatMap { TrackFastGDNReplay.make(hc: attnHC, gdn: $0,
                hcCount: cfg.hcCount, hidden: cfg.hiddenSize) }, ple: ple)
    }
    ...
    func fastStreams(
        ...
        for layer in layers {
            ...
            if TrackFastGDNReplay.enabled, !capture, ids.shape == [1, 1], normed.dtype == .bfloat16,
                !profiling, Self.debugTaps == nil, StreamOrDevice.default.stream === Stream.gpu,
                let replay = layer.gdnReplay, let g = layer.gdn
            {
                let state = evaluation.inputState(modelLayerIndex: layer.index)
                let conv = state?.conv ?? MLXArray.zeros([1, g.geometry.convKernel - 1, g.geometry.convDim], dtype: normed.dtype)
                let ssm = state?.ssm ?? MLXArray.zeros([1, g.geometry.hv, g.geometry.dv, g.geometry.dk], dtype: .float32)
                let r = replay([normed, conv, ssm])
                attended = r[0]; injectW = r[1]
                // No debug consumer is active in this branch. The MLP below
                // replaces input before consuming it.
                input = normed
                do { try evaluation.stage(modelLayerIndex: layer.index, conv: r[2], ssm: r[3]) }
                catch { preconditionFailure("TrackFastModel: recurrent replay stage failed at layer \(layer.index): \(error)") }
            } else {
