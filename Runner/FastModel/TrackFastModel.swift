...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func pleForward(
        ...
        // MLXFAST-PLEFUSE3: the same two-launch epilogue for the captured
        // verify window (S = 2...7); every other shape takes the three-launch
        // PLE block below.
        let full: MLXArray
        let output: MLXArray
                ...
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            ...
            TrackPLEFusion.supportsWindow(p, stream: stream, hidden: hidden, hcCount: hcCount),
            convState.shape == [1, p.stateLength, wide], convState.dtype == stream.dtype,
            let fused = TrackPLEFusion.forwardWindow(
                p, embedded: embedded, stream: stream, convState: convState, eps: eps)
        {
            (full, output) = fused
        } else {
