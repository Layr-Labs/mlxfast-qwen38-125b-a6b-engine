...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    /// Profiling: when set, wide windows (prefill) eval after every block and
    /// accumulate wall time per block kind here (harness only).
    nonisolated(unsafe) public static var prefillProfile: [String: Double]? = nil
    static func profTick(_ key: String, _ t0: inout Double, _ arrays: [MLXArray]) {
        guard prefillProfile != nil else { return }
        eval(arrays)
        let t = CFAbsoluteTimeGetCurrent()
        prefillProfile![key, default: 0] += t - t0
        t0 = t
    }
    ...
    private func gdnForward(
        ...
        let geo = g.geometry
        let prof = Self.prefillProfile != nil && S > 8
        var pt = CFAbsoluteTimeGetCurrent()
        let proj = g.proj.apply(x)  // [B,S,PROJ_W]
        if prof { Self.profTick("gdn.proj", &pt, [proj]) }
        ...
        if prof { Self.profTick("gdn.prep+lean", &pt, [r.y, r.stateOut, r.convOut]) }
        let gated = TrackFastKernels.gatedRMS(
            y: r.y, proj: proj, w: g.normW, zOffset: g.zOffset, eps: 1e-6)
        if prof { Self.profTick("gdn.gatedRMS", &pt, [gated]) }
        do {
            if capture {
        ...
        if prof { Self.profTick("gdn.out", &pt, [o]) }
        return o
    }
    ...
    static func moeForwardShared(_ m: TrackMoE, _ x: MLXArray) -> MLXArray {
        let prof = Self.prefillProfile != nil && x.dim(1) > 8
        var pt = CFAbsoluteTimeGetCurrent()
        let logits = matmul(x.asType(.float32), m.routerW32.transposed())
        if prof { Self.profTick("moe.router", &pt, [logits]) }
        // Top-k + softmax in one launch (argpartition's stable order, softmax_single_row).
        if x.dim(0) == 1, x.dim(1) <= 8,
            case .quant(let gq) = m.sharedGate, case .quant(let guq) = m.sharedGateUp.fused ?? .dense(x),
            case .quant(let dq) = m.sharedDown, gq.biases != nil, guq.biases != nil, dq.biases != nil
        {
            ...
            let (idx, weights, gate) = TrackFastMoEKernels.route(
                logits: logits.reshaped(S, -1), x: x2, sharedGate: gq, topK: K)
            if prof { Self.profTick("moe.route", &pt, [idx, weights, gate]) }
        ...
        if prof { Self.profTick("moe.route", &pt, [idx, weights]) }
        ...
        if prof { Self.profTick("moe.shared", &pt, [shared, gate]) }
        let routed = m.switchMLP(x, idx)  // [B,S,K,H]
        if prof { Self.profTick("moe.switchMLP", &pt, [routed]) }
    ...
    func fastStreams(
        ...
        let profiling = Self.prefillProfile != nil && ids.dim(1) > 8
        if Self.prefillProfile != nil { print("[profile] fastStreams S=\(ids.dim(1)) profiling=\(profiling)") }
        for layer in layers {
            var normed: MLXArray
            ...
            tile = false
            if profiling { Self.profTick(layer.ple != nil ? "norm+ple" : "norm", &profT, [stream, normed]) }
            var (input, injectW) = hcMix(layer.attnHC, normed: normed, tag: "L\(layer.index).attn.hc")
            if profiling { Self.profTick("mixer", &profT, [input, injectW]) }
            ...
            if profiling { Self.profTick(layer.gdn != nil ? "gdn" : "attn", &profT, [attended]) }
            (stream, normed) = TrackFastKernels.injectNorm(
                residual: stream, out: attended, inject: injectW,
                scale: layer.mlpHC.normScaleQ, hcCount: hcCount, hidden: hidden, eps: eps,
                tile: false)
            if profiling { Self.profTick("norm", &profT, [stream, normed]) }
            ...
            if profiling { Self.profTick("moe", &profT, [pendingOut!]) }
