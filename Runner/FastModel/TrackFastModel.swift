...
private final class TrackWholeGDNParameters {
    private let arrays: [MLXArray]
    private let placeholder = MLXArray.mlxNone
    private let lock = NSLock()
    private var arguments: [MLXArray]

    init(arrays: [MLXArray]) {
        precondition(arrays.count == 50)
        self.arrays = arrays
        self.arguments = []
        arguments.reserveCapacity(55)
        arguments.append(contentsOf: [placeholder, placeholder, placeholder])
        arguments.append(contentsOf: arrays)
        arguments.append(contentsOf: [placeholder, placeholder])
    }

    func call(
        _ replay: @Sendable ([MLXArray]) -> [MLXArray], residual: MLXArray,
        previousOut: MLXArray, previousInject: MLXArray, conv: MLXArray, ssm: MLXArray
    ) -> [MLXArray] {
        guard lock.try() else {
            var inputs: [MLXArray] = []
            inputs.reserveCapacity(55)
            inputs.append(residual)
            inputs.append(previousOut)
            inputs.append(previousInject)
            inputs.append(contentsOf: arrays)
            inputs.append(conv)
            inputs.append(ssm)
        ...
        defer {
            ...
            arguments[1] = placeholder
            arguments[2] = placeholder
            arguments[53] = placeholder
            arguments[54] = placeholder
            lock.unlock()
        }
        arguments[0] = residual
        arguments[1] = previousOut
        arguments[2] = previousInject
        arguments[53] = conv
        arguments[54] = ssm
...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    private func wholeGDNLayer(
        ...
        let result = parameters.call(
            replay.call, residual: residual, previousOut: previousOut,
            previousInject: previousInject, conv: conv, ssm: ssm)
