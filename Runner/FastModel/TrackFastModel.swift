...
struct TrackMultiProj {
    ...
    func apply(_ x: MLXArray) -> MLXArray {
        if let fused { return fused.apply(x) }
...
struct TrackHC {
    ...
    /// input_mix_weight_down rows, then block_inject_weight rows when present.
    let downInject: TrackMultiProj
    let up: TrackProj
    let lowrank: Int
    let hasInject: Bool
}
...
struct TrackAttn {
    /// q rows (all heads), gate rows (all heads), k rows, v rows.
...
public final class TrackQwen4ExpFastModel: Module, @unchecked Sendable {
    ...
    static func bindHC(_ m: Module, cfg: Qwen4ExpTextConfiguration) -> TrackHC {
        ...
        let multi = TrackMultiProj(inject == nil ? [down] : [down, inject!])
        let q = (scale * MLXArray(Float(1) / Float(cfg.hcCount), dtype: scale.dtype))
        return TrackHC(
            normScaleQ: q, downInject: multi, up: up, lowrank: cfg.hcLowrank,
    ...
    static func bindAttn(_ m: Module, cfg: Qwen4ExpTextConfiguration) -> TrackAttn {
        ...
        let qkv = TrackMultiProj([qReordered, k, v])
        ...
        return TrackAttn(
            qkv: qkv, qNormW: qNormW, kNormW: kNormW, indexerK: indexerK, out: out,
    ...
    static func bindMoE(_ m: Module, cfg: Qwen4ExpTextConfiguration) -> TrackMoE {
        ...
        return TrackMoE(
            routerW32: routerW, switchMLP: switchMLP, sharedGateUp: TrackMultiProj([sg, su]),
    ...
    /// Hyper-connection mixer: returns the block input `[B,S,H]` and the
    /// inject weights `[B,S,hc]` (nil for the final mixer).
    private func hcMix(_ hc: TrackHC, _ stream: MLXArray) -> (MLXArray, MLXArray?) {
        let lead = Array(stream.shape.dropLast())
        let normedQ = groupNorm(stream, scale: hc.normScaleQ)  // normed / hc
        let lo = hc.downInject.apply(normedQ)
        let loMix = hc.hasInject ? lo[.ellipsis, ..<hc.lowrank] : lo
        let w = sigmoid(hc.up.apply(silu(loMix)))
        let input =
            (w.reshaped(lead + [hcCount, hidden]) * normedQ.reshaped(lead + [hcCount, hidden]))
            .sum(axis: -2)
        let inject = hc.hasInject ? 2 * sigmoid(lo[.ellipsis, hc.lowrank...]) : nil
        return (input, inject)
    }

    private func inject(_ residual: MLXArray, _ output: MLXArray, _ inject: MLXArray) -> MLXArray
    {
        let lead = Array(output.shape.dropLast())
    ...
    private func gdnForward(
        ...
        let z = proj[.ellipsis, g.zOffset ..< (g.zOffset + g.valueDim)]
            .reshaped(B, S, geo.hv, geo.dv)
        let normed = MLXFast.rmsNorm(r.y, weight: g.normW, eps: 1e-6)
        let gated = (sigmoid(z.asType(.float32)) * normed.asType(.float32)).asType(x.dtype)
        do {
            if capture {
        ...
        return g.out.apply(gated.reshaped(B, S, -1))
    }

    private func attnForward(
        _ a: TrackAttn, _ x: MLXArray, cache: Qwen4ExpCBv2LayerCache, offset: Int
    ) -> MLXArray {
        let B = x.dim(0), S = x.dim(1)
        let heads = cfg.attentionHeads, kvHeads = cfg.kvHeads, d = cfg.headDim

        // The indexer tape first: its truncation reads the pre-update offset.
        let idxKeys = a.indexerK.apply(x).reshaped(B, S, cfg.indexerHeadDim)
        _ = cache.updateIndexerTape(keys: idxKeys)

        let qkv = a.qkv.apply(x)
        let qw = a.qWidth, kw = a.kvWidth
        let q = qkv[.ellipsis, ..<qw].reshaped(B, S, heads, d)
        let gate = qkv[.ellipsis, qw ..< (2 * qw)]
        let k = qkv[.ellipsis, (2 * qw) ..< (2 * qw + kw)].reshaped(B, S, kvHeads, d)
        let v = qkv[.ellipsis, (2 * qw + kw)...].reshaped(B, S, kvHeads, d)

        var queries = MLXFast.rmsNorm(q, weight: a.qNormW, eps: eps).transposed(0, 2, 1, 3)
        var keys = MLXFast.rmsNorm(k, weight: a.kNormW, eps: eps).transposed(0, 2, 1, 3)
        let values = v.transposed(0, 2, 1, 3)
        queries = MLXFast.RoPE(
            queries, dimensions: rotaryDims, traditional: false, base: cfg.ropeTheta,
            scale: 1.0, offset: offset)
        keys = MLXFast.RoPE(
            keys, dimensions: rotaryDims, traditional: false, base: cfg.ropeTheta,
            scale: 1.0, offset: offset)
        ...
        return a.out.apply(out * sigmoid(gate))
    }
    ...
    private func moeForward(_ m: TrackMoE, _ x: MLXArray) -> MLXArray {
        ...
        let routed = (m.switchMLP(x, idx) * weights[.ellipsis, .newAxis])
            .sum(axis: -2)
            .asType(x.dtype)
        let gu = m.sharedGateUp.apply(x)
        let h = m.sharedHidden
        let shared = m.sharedDown.apply(silu(gu[.ellipsis, ..<h]) * gu[.ellipsis, h...])
        return routed + sigmoid(m.sharedGate.apply(x)) * shared
    }
    ...
    func fastStreams(
        ...
        var stream = inputEmbeddings ?? embedTokens(ids)
        stream = tiled(stream, repetitions: [1, 1, hcCount])
        ...
        for layer in layers {
            ...
            var (input, injectW) = hcMix(layer.attnHC, stream)
            ...
            if let gdn = layer.gdn {
                ...
                attentionIndex += 1
                attended = attnForward(layer.attn!, input, cache: cache, offset: offset)
            }
            stream = inject(stream, attended, injectW!)
            (input, injectW) = hcMix(layer.mlpHC, stream)
            stream = inject(stream, moeForward(layer.moe, input), injectW!)
        }
        let (mixed, _) = hcMix(finalMixer, stream)
        return (mixed, stream)
    }
