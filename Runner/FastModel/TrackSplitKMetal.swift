// LIFTED VERBATIM from the vendored MLX JIT sources -- do not hand-edit.
//
// Regenerate by extracting the `R"preamble( ... )preamble"` body of
// Vendor/mlx-swift/Source/Cmlx/mlx-generated/{gemm,quantized_utils,quantized}.cpp
// and concatenating them in that order. Editing this file by hand would
// defeat its purpose: the point is that the arithmetic below is MLX's own,
// not a reimplementation of it.
//
// The Metal preamble for a Runner kernel that instantiates MLX's own
// `qmm_t_impl`, so a fused split-K matmul computes each output element with
// the SAME code, not merely the same intent. Copied verbatim from the vendored
// JIT sources, in the order `jit_kernels.cpp:1029-1039` concatenates them for
// an affine quantized kernel:
//
//     metal::utils() + metal::gemm() + metal::quantized_utils()
//         + metal::quantized()
//
// `metal::utils()` is omitted because a custom kernel's library source is
// already `metal::utils() + source` (`backend/metal/custom_kernel.cpp:53`).
//
// Sources and byte counts at generation time:
//   mlx-generated/gemm.cpp  57337 bytes
//   mlx-generated/quantized_utils.cpp  2783 bytes
//   mlx-generated/quantized.cpp  108364 bytes
enum TrackSplitKMetal {
    static let header = #"""
