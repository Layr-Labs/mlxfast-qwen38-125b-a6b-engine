...
final class TrackPlePrefetch {
    ...
    /// The window a kicked job belongs to. `awaitResult` refuses a job whose
    /// window does not match the caller's, so a `start` that is never awaited
    /// (an early return before the PLE layer) can never hand a LATER window a
    /// stale history: the result would be wrong rows, wrong tokens and a failed
    /// correctness gate rather than a lost draw. Falls back to the inline path.
    private var pendingKey: (offset: Int, count: Int)?
    ...
    /// Await the kicked job for THIS window. Called from the dispatch thread
    /// before the mirror is stored. A job stamped for a different window is
    /// drained and discarded, and the caller takes its existing inline path.
    func awaitResult(offset: Int, count: Int) -> Result? {
        guard let work = pending else { return nil }
        work.wait()
        pending = nil
        let key = pendingKey
        pendingKey = nil
        guard let key, key.offset == offset, key.count == count else {
            result = nil
            return nil
        }
        let r = result
