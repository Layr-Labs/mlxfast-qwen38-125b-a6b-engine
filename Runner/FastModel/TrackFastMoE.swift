...
extension TrackFastMoEKernels {
    ...
    /// Callers bind `row`, `lane`, `sg`, `E_PER`, `VPT`, `HAS_GATE`.
    ...
    /// logits f32 [R, E] -> idx uint32 [R, K], w f32 [R, K], gate T [R].
    ...
    /// weight binds, no unused gate buffer. `VPT = 2, HAS_GATE = false` selects
    /// the register-resident walk (REGISTER_RESULTS, SEL_SG = 0): one simdgroup
    /// does the whole top-k and softmax with no threadgroup round trip.
    static let prefillRouteSource = """
        constexpr int VPT = 2;
        constexpr bool HAS_GATE = false;
        constexpr int E_PER = (E + 31) / 32;
        const uint row = threadgroup_position_in_grid.y;
        const uint lane = thread_index_in_simdgroup;
        const uint sg = simdgroup_index_in_threadgroup;
        ...
        -> (idx: MLXArray, w: MLXArray, gate: MLXArray)
    {
        ...
        let outs = (R == 1 ? routeKernel1 : routeKernel)(
            ...
            template: [("E", E), ("K", topK), ("T", x.dtype), ("GS", g?.groupSize ?? 32), ("BITS", g?.bits ?? 4), ("KD", KD), ("VPT", g == nil ? 2 : R), ("HAS_GATE", g != nil)],
    ...
    nonisolated(unsafe) static let prefillRouteKernel = MLXFast.metalKernel(
        ...
        inputNames: ["logits"],
        outputNames: ["idx", "w"],
        source: prefillRouteSource, header: TrackFastKernels.mixerHeadHeader + wideDecls, ensureRowContiguous: true)

    /// logits f32 [..., E] -> (idx uint32 [..., K], w f32 [..., K]). Prefill only:
    /// no shared gate, so no x/weight binds and no gate output.
