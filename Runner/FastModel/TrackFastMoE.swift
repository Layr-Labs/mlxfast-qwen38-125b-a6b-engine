    ...
    static let gateUpActSource = """
        ...
        if (z == (uint)BR) {
            ...
            if constexpr (VPT == 1) {
                ...
                // MLXFAST-WIDEHEAD: `qmv_wide_reg_full` zeroes and accumulates
                // every one of its `vecs_per_tg` (= VPT) vectors on EVERY lane
                // of the 8-lane group before the cross-lane reduction, so after
                // it each lane of the group holds all VPT results identically.
                // The `% 8 == 0` test was only choosing a store leader. Each
                // lane stores its own token instead. VPT <= 8 at this call site
                // (see the precondition above), so the 8-lane group covers every
                // token exactly once and each write goes to a distinct address.
                // Same values, same arithmetic, same addresses.
                if ((thread_index_in_simdgroup % 8) < VPT) {
                    const int v = (int)(thread_index_in_simdgroup % 8);
...
/// submission. For this track that is `mtp-head.manifest.json`, `Runner/`,
/// `Sources/MLXFastTransform/`, and `Vendor/mlx-swift/Source/Cmlx/mlx-generated/
/// *.cpp` -- note the generated `.cpp` files, NOT the `.h` files they are built
/// from, and NOT `Vendor/mlx-swift/Source/MLX/` (the Swift front end). A change
