...
extension TrackFastMoEKernels {
    ...
    static let oProjSource = """
        ...
        const uint sg  = simdgroup_index_in_threadgroup;
        const uint lid = thread_index_in_simdgroup;
        // RPS (rows per simdgroup) is a template constant. MLX ships 4.
    ...
    static func oProj2Row(_ out: MLXArray, _ q: TrackQuantWeight, rps: Int = 2) -> MLXArray? {
        let K = out.dim(-1), N = q.rows
        guard out.size == K, out.dtype != .float32, q.bits == 4,
            q.mode == .affine, let bd = q.biases, N % (2 * rps) == 0,
        ...
        let tiles = N / (2 * rps)  // 2 simdgroups * rps rows each
