...
extension TrackFastMoEKernels {
    ...
    static func gateUpAct(
        ...
        // `isFast(k:n:)` is a pure function of KD and N, evaluated twice on
        // this path (precondition and `FAST` template value) with neither
        // rebound in between. Bind it once; the template receives the same
        // Bool, so kernel selection and every byte moved are unchanged.
    ...
    static func downCombine(
        ...
        // `isFast(k:n:)` is a pure function of F and H, evaluated twice on
        // this path (precondition and `FAST` template value) with neither
        // rebound in between. Bind it once; the template receives the same
        // Bool (the precondition asserts it is false).
