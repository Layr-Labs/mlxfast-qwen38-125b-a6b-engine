...
extension TrackFastMoEKernels {
    ...
    /// MEASURED AND FALSIFIED (local paired decode A/B, 16 interleaved pairs).
    /// Value 1 was tried against the shipped 2 and showed NO EFFECT: run 1 gave
    /// +118 us (gate=1 faster, t=+1.72, 7/8 pairs), run 2 gave -90 us (gate=1
    /// slower, t=-0.68, 3/8 pairs). Combined: +14 us on a 17.6 ms step, t=+0.18,
    /// 10/16 -- a null. Per-pair sd is ~306 us, so resolving a 0.67% effect needs
    /// ~27 pairs per arm, not 8. The first run alone would have fooled me.
    ///
    /// Why local decode A/Bs are valid at all: this artifact ships with MTP
    /// DISABLED (mtp-head.manifest.json spec.enabled = false), so the ranked
    /// decode runs this same serial S == 1 path. Decode is the only leg where
    /// local numbers transfer; the wide (S > 1) branch is never on the scored
    /// path, and local prefill noise is 10-20%.
    ///
    /// The claim below that "= 4 lost ~1.5%" was judged from one or two RANKED
    /// draws, whose noise is ~0.8%, so it was never a measurement either. Treat
    /// every constant verdict in this file as unmeasured until a paired local
    /// A/B with n >= 27 per arm says otherwise.
    ...
    /// `downCombineSimdgroups = 4`: NOW MEASURED and confirmed -- a paired local
    /// decode A/B over 24 interleaved pairs gives dc4 slower by 168.6 us on a
    /// 17.6 ms step = 0.96%, t = -2.83, with 4 faster in only 5 of 24 pairs. The
    /// shipping value 5 is correct; this comment previously asserted it from one
    /// or two RANKED draws and had never been measured.
    ...
    /// hardcoded 4.
    ///
    /// MEASURED SINCE, with paired and counterbalanced local A/Bs. Only the
    /// decode leg transfers, because MTP is disabled here so the ranked
    /// candidate leg runs this same serial S == 1 path:
    ///
    ///   downCombineSimdgroups 5 vs 4      -> 5 wins by 0.93%, t = -2.58, n = 28.
    ///                                        Confirmed; value unchanged.
    ///   gateUpReuseRowsPerSimdgroup 2 vs 1 -> null (t = +0.18, n = 16). This does
    ///                                        not test the "= 4" claim above,
    ///                                        which compares a different pair.
    ///   TrackFastMixerSplitK.split 4 vs 0  -> null; the lean decayed from 0.74%
    ///                                        at n = 5 to 0.33% at n = 14.
    ///   TrackP12Prefill sortedCombine on/off -> null on prefill.
    ///
    /// RESOLUTION FLOOR, which bounds how far tuning can go: decode per-pair sd
    /// is ~1.9% of the step, so a 28-pair A/B settles effects of ~0.7% and
    /// larger and nothing smaller (a 0.5% effect needs ~111 pairs). Effects
    /// below that line are unresolved, not absent. Prefill per-pair sd is ~9.4%,
    /// so local prefill A/Bs settle only >= 10% effects.
