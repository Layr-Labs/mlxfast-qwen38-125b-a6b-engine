...
extension TrackFastMoEKernels {
    ...
    static let routeSource = """
        ...
        if constexpr (VPT == 1 && HAS_GATE) {
            // MLXFAST-ROUTEASYNC: for one token with shared-gate GEMV, simdgroup 0
            // executes track_inject_qmv (writing gate[row]) and returns immediately.
            // Simdgroup 1 executes the top-K walk and softmax completely in registers.
            // Since simd_max and simd_min broadcast to all lanes in simdgroup 1,
            // the K selected values and indices are held in private registers across
            // lanes, entirely bypassing threadgroup memory and the threadgroup barrier.
            if (sg == 0) { return; }
