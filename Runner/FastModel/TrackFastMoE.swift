...
extension TrackFastMoEKernels {
    ...
    /// Default 4. Local M4 Max A/B: RPS=2 was slower (decode 0.85×, prefill 0.93×).
    ...
    /// One-token occupancy. Wide windows stay at 4. `override` wins over the
    /// env kill switch so one process can compare 2 vs 4.
    static func downCombineRowsPerThreadgroup(
        windowTokens: Int,
        override: Int? = nil,
        env: String? = ProcessInfo.processInfo.environment["MLXFAST_MOE_DOWN_RPS"]
    ) -> Int {
        windowTokens == 1 ? (override ?? (env.flatMap { Int($0) } ?? downRowsPerSimdgroup)) : 4
    }
    ...
    /// `staging` overrides TRACK_WEIGHT_STAGING (tests). Nil uses the toggle.
    /// `rowsPerThreadgroup` overrides the one-token occupancy (tests). Nil uses
    /// `MLXFAST_MOE_DOWN_RPS` or 4. Wide windows stay at 4.
    static func downCombine(
        ...
        let rps = downCombineRowsPerThreadgroup(windowTokens: S, override: rowsPerThreadgroup)
        let dummy = act
        let tiles = max(fusedTiles, 1)
