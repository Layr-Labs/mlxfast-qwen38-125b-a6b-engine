...
extension TrackFastMoEKernels {
    ...
    /// The caller binds `row`, `lane`, `sg`, `E_PER`, `logits`, `idx`, `w` and
    /// the `VPT` / `HAS_GATE` template constants.
    ...
    nonisolated(unsafe) static let prefillRouteKernel = MLXFast.metalKernel(
        ...
        inputNames: ["logits"],
        outputNames: ["idx", "w"],
        source: prefillRouteSource, header: TrackFastKernels.mixerHeadHeader + wideDecls,
        ensureRowContiguous: true)

    /// logits f32 [..., E] -> (idx uint32 [..., K], w f32 [..., K]). Prefill only:
    /// VPT = 2 keeps the register-results path (SEL_SG = 0) on one simdgroup.
