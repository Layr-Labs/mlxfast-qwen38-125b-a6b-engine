...
extension TrackFastMoEKernels {
    ...
    /// `row`, `lane`, `sg`, `E_PER`, `VPT`, `HAS_GATE`.
    ...
    /// weight binds, no unused gate output, no VPT=R specialization.
    ...
    /// the shared-expert gate is computed separately there, so this kernel binds
    /// no x/weight buffers and emits no gate row.
