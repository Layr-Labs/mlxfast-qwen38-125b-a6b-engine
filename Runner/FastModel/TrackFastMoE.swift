...
extension TrackFastMoEKernels {
    ...
    static func downCombine(
        ...
        // MLXFAST-DCFAST: the same double evaluation on the down-combine path,
        // over F and H -- `isFast(k:n:)` is pure, so one bind serves the
        // precondition and the `FAST` template value with the same Bool.
        let dfast = isFast(k: F, n: H)
        precondition(act.dim(0) == BR + S && gate.dim(0) == S && sharedDown.rows == H && !dfast)
        ...
        return (S == 1 ? downCombineKernel1 : downCombineKernel)(
            ...
            template: [("T", act.dtype), ("GS", groupSize), ("BITS", bits), ("H", H), ("F", F), ("K", topK), ("FAST", dfast), ("BR", BR), ("VPT", S), ("KSG", ksg), ("RPS", rps)],
