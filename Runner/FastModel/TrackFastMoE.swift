...
extension TrackFastMoEKernels {
    ...
    /// Routed experts, gate|up + SwiGLU. x [S, KD], idx uint32 [BR] ->
    ...
    /// One-token MoE gate|up specialization. The guard in `gateUpAct` fixes the
    /// activation dtype, KD, N, group size and bit width, so the geometry is
    /// inline here instead of being marshalled into template arguments.
    static let gateUpReuseSource = """
        constexpr int KD = 2560;
        constexpr int N = 640;
        constexpr int GS = 32;
        constexpr int BITS = 4;
        constexpr int RPS = \(gateUpReuseRowsPerSimdgroup);
        const uint z = threadgroup_position_in_grid.z;
        const bool shared = z == (uint)BR;
        const uint e = shared ? 0u : idx[z];
        const uint r = shared ? 0u : z / (uint)BR;
    ...
    static func gateUpAct(
        wg: MLXArray, sg: MLXArray, bg: MLXArray, wu: MLXArray, su: MLXArray, bu: MLXArray,
        shared: TrackQuantWeight, x: MLXArray, idx: MLXArray, groupSize: Int, bits: Int
    ) -> MLXArray {
        let BR = idx.dim(0), S = x.dim(0), KD = x.dim(1), N = wg.dim(1)
        precondition(N % 8 == 0 && bits == 4 && idx.dtype == .uint32 && BR % S == 0)
