...
extension TrackFastMoEKernels {
    ...
    /// MEASURED (local paired decode A/B, n=8 interleaved pairs): value 1 beats
    /// the shipped value 2 by a mean 118 us = 0.67% of the decode step, winning
    /// 7 of 8 pairs (paired t = 1.72, df = 7 -- suggestive, not yet significant
    /// at 5%). This is measurable locally at all only because MTP is disabled,
    /// so the ranked decode runs this same serial S == 1 path; local decode noise
    /// is ~1%. The earlier note here that "gateUpReuseRowsPerSimdgroup = 4 lost
    /// ~1.5%" was judged from one or two RANKED draws, whose noise is ~0.8%, so
    /// it was never a measurement -- which is why 1 had not been tried.
