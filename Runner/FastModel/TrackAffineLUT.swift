...
struct TrackAffineLUT {
    ...
    /// The routed expert stacks (48 x 3 tensors, +8 GB of u16 indices beside
    /// the bf16 metadata the prefill kernels still read) are indexed only on
    /// request (env `TRACK_AFFINE_LUT_EXPERTS=1`); the default serves the dense
    /// projections and the head, which add no resident memory to speak of.
    nonisolated(unsafe) static var expertsEnabled: Bool =
        ProcessInfo.processInfo.environment["TRACK_AFFINE_LUT_EXPERTS"] != "0"
    /// Once an expert stack's index is built and verified, its bf16 scale and
    /// bias arrays are replaced by lazy reconstructions from the table (the
    /// same bits, materialized only by a path that still reads them: wide
    /// windows, the reference fallbacks), so the index costs no resident
    /// memory: 2 bytes per group instead of 4. Env `TRACK_AFFINE_LUT_KEEP_BF16=1`
    /// keeps the loaded arrays resident beside the index.
    nonisolated(unsafe) static var releaseBF16: Bool =
        ProcessInfo.processInfo.environment["TRACK_AFFINE_LUT_KEEP_BF16"] != "1"
    /// The bf16 scales this index encodes, as an unevaluated graph over the
    /// index and the table (bit-identical to the loaded array).
    func lazyScales() -> MLXArray {
        (table[index.asType(.int32)] & MLXArray(UInt32(0xffff))).asType(.uint16).view(dtype: .bfloat16)
    }
    /// The bf16 biases this index encodes, same construction.
    func lazyBiases() -> MLXArray {
        (table[index.asType(.int32)] >> MLXArray(UInt32(16))).asType(.uint16).view(dtype: .bfloat16)
    }
