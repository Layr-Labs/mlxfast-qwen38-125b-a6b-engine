...
enum TrackFastGDNDecode {
    ...
    private static let source = #"""
        ...
        // Gate scalars depend only on proj/neg_exp_alog/dt_bias, not on the
        // conv staging. Simdgroups >= 3 are idle until the barrier, so lane 0
        // of simdgroup 12 produces the identical values while the conv groups
        // work; simdgroup 0 reaches the barrier sooner.
        if (sg == 12 && lane == 0) {
