...
enum TrackFastGDNDecode {
    ...
    private static let source = #"""
        ...
        if (sg < 3) {
            ...
            if constexpr (KC == 4) {
                const auto s0 = *reinterpret_cast<const device metal::vec<InT, 4>*>(cst_b + ch0);
                const auto s1 = *reinterpret_cast<const device metal::vec<InT, 4>*>(cst_b + CONV_DIM + ch0);
                const auto s2 = *reinterpret_cast<const device metal::vec<InT, 4>*>(cst_b + 2 * CONV_DIM + ch0);
                const auto s3 = *reinterpret_cast<const device metal::vec<InT, 4>*>(proj_b + ch0);
                const float4 t0 = float4(s0);
                const float4 t1 = float4(s1);
                const float4 t2 = float4(s2);
                const float4 t3 = float4(s3);
                for (int i = 0; i < 4; ++i) {
                    const auto w4 = float4(*reinterpret_cast<const device metal::vec<InT, 4>*>(
                        conv_w + (ch0 + i) * 4));
                ...
                if (sg < 2) {
                    ...
                    if (sg == 0) {
                        *reinterpret_cast<threadgroup metal::vec<InT, 4>*>(q_shared + lane * 4) = n4;
                    } else {
                        *reinterpret_cast<threadgroup metal::vec<InT, 4>*>(k_shared + lane * 4) = n4;
                    }
                } else {
                    *reinterpret_cast<threadgroup metal::vec<InT, 4>*>(v_shared + lane * 4) = metal::vec<InT, 4>(
                ...
                if (sg == 2 || hv_idx % (Hv / Hk) == 0) {
                    ...
                    *reinterpret_cast<device metal::vec<InT, 4>*>(o_conv + ch0) = s1;
                    *reinterpret_cast<device metal::vec<InT, 4>*>(o_conv + CONV_DIM + ch0) = s2;
                    *reinterpret_cast<device metal::vec<InT, 4>*>(o_conv + 2 * CONV_DIM + ch0) = s3;
                }
            } else {
                for (int i = 0; i < 4; ++i) {
                    const uint ch = ch0 + i;
        ...
        if (sg == 0) {
            ...
            const auto w4 = *reinterpret_cast<const device metal::vec<InT, 4>*>(w + lane * 4);
            const auto z4 = *reinterpret_cast<const device metal::vec<InT, 4>*>(proj + (b_idx * PW + Z_OFF + hv_idx * Dv + lane * 4));
            metal::vec<InT, 4> out4;
            for (int i = 0; i < 4; ++i) {
            ...
            *reinterpret_cast<device metal::vec<InT, 4>*>(gated + (n * Dv + lane * 4)) = out4;
        }
        """#
}
