...
enum TrackFastGDNDecode {
    ...
    static func apply(
        ...
        // evaluated four times: the `convState.dtype` comparison below, the
        // `InT` template value, and both `proj` entries of `outputDTypes`.
        // Binding it once turns four reads into one; the same `DType` fills
        // the same slots, so the kernel choice, every template constant, the
        // grid, the threadgroup shape and every byte moved are unchanged.
