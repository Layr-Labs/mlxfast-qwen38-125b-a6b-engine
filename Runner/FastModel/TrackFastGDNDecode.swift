...
enum TrackFastGDNDecode {
    ...
    private static let source = #"""
        ...
        // Issue sg==0's immutable w/proj reads while peer simdgroups finish
        // y_shared. The existing barrier still protects every y_shared read;
        // these private values cross it unchanged into the original epilogue.
        vec<InT, 4> w4_early;
        vec<InT, 4> z4_early;
        if (sg == 0) {
