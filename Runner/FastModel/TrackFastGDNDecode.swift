...
enum TrackFastGDNDecode {
    ...
    static func apply(
        ...
        // MLXFAST-GDNJ3: the journal carries up to three pending rank-1
        // updates. Its allocation shape is fixed at three slots so the
        // deferred steps never change the buffer geometry; the flush happens
        // on the fourth step, which is the only step that writes the
        // recurrent state back to device memory.
        let depth = pendingJournal?.shape == [B, 3 * journalStride]
            && pendingJournal?.dtype == proj.dtype && (1...3).contains(pendingDepth)
            ? pendingDepth : 0
        let flush = depth == 3
    ...
    private static let source = #"""
        ...
        static_assert(J_DEPTH >= 0 && J_DEPTH <= 3, "GDN journal depth");
        ...
        if constexpr (J_DEPTH > 0) { journal += b_idx * (3 * J_STRIDE); }
        // A flush has only a placeholder journal output; form no offset into it.
        device InT* next_journal = journal_out;
        if constexpr (J_DEPTH < 3) { next_journal += b_idx * (3 * J_STRIDE) + J_DEPTH * J_STRIDE; }
        ...
        for (int r = 0; r < RPS; ++r) {
            ...
            if constexpr (J_DEPTH < 3) {
                if (lane == 0) {
                    const int off = J_DELTA_OFF + 2 * (hv_idx * Dv + dv_idx);
                    store_journal_float(off, delta);
                    // Carry the still-pending slots forward unchanged.
                    for (int p = 0; p < J_DEPTH; ++p) {
                        const int shift = (p - J_DEPTH) * J_STRIDE;
                        next_journal[off + shift] = journal[p * J_STRIDE + off];
                        next_journal[off + 1 + shift] = journal[p * J_STRIDE + off + 1];
                    }
                }
            }
            float out = 0.0f;
        ...
        if constexpr (J_DEPTH < 3) {
            if (sg == 0) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    next_journal[off] = k_[4 * lane + i];
                    for (int p = 0; p < J_DEPTH; ++p) {
                        next_journal[off + (p - J_DEPTH) * J_STRIDE] = journal[p * J_STRIDE + off];
                    }
                }
            }
            if (sg == 0 && lane == 0) {
                const int off = J_DECAY_OFF + 2 * hv_idx;
                store_journal_float(off, gate_decay);
                for (int p = 0; p < J_DEPTH; ++p) {
                    const int shift = (p - J_DEPTH) * J_STRIDE;
                    next_journal[off + shift] = journal[p * J_STRIDE + off];
                    next_journal[off + 1 + shift] = journal[p * J_STRIDE + off + 1];
