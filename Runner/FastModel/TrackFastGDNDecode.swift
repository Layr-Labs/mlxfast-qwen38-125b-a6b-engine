...
enum TrackFastGDNDecode {
    ...
    private static let source = #"""
        ...
        // dt_bias — not on the conv staging or the norm/copy phase — but they
        // used to run on simdgroup 0 *after* its post-barrier slice,
        // serializing them behind work they do not consume. Simdgroups >= 12
        // are idle through both phases; lane 0 of group 12 produces the
        // identical values while the conv groups work, so the scalars are
        // staged earlier and simdgroup 0 reaches the barrier sooner.
        if (sg == 12 && lane == 0) {
