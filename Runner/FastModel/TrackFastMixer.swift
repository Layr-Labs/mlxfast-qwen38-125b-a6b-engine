...
enum TrackFastMixerKernels {
    ...
    // MLXFAST-UPRPS: packed rows per simdgroup in the one-token upMix. A tile
    // spans `upPackColumns` hidden columns, so `decodeUp` must group its rows
    // the same way; 2 halves the tile width of the original 4.
    static let upRowsPerSimdgroup = 2
    static var upPackColumns: Int { upRowsPerSimdgroup / 2 }
    ...
    /// MLXFAST-UPRPS: tile t owns COLS hidden columns across the HC streams
    /// (HC * COLS rows); slot s -> row (COLS * t + s % COLS) + H * (s / COLS),
    /// matching the `decodeUp` row order. grid threads (32, 2 * H/COLS, 1),
    /// tg (32, 2, 1). COLS = UPRPS / 2, so a smaller UPRPS is a narrower tile
    /// and proportionally more threadgroups over the same rows.
    static let upMixSource = """
        ...
        constexpr int COLS = UPRPS / 2;
        const int tile = (int)threadgroup_position_in_grid.y;
        const int d0 = COLS * tile;
        const uint sg = simdgroup_index_in_threadgroup;
        const uint lid = thread_index_in_simdgroup;
        threadgroup T products[HC * COLS][VPT];
        if constexpr (VPT == 1) {
            float r[UPRPS];
            if constexpr (PACKED_ROWS) {
                qmv_reg<T, GS, BITS, (LW % get_pack_factor<BITS, 32>()) == 0, UPRPS>(wu, su, bu, act, LW, tile * (HC * COLS) + (int)sg * UPRPS, lid, r);
            } else {
                int rows[UPRPS];
                for (int i = 0; i < UPRPS; ++i) { const int s = (int)sg * UPRPS + i; rows[i] = d0 + (s % COLS) + H * (s / COLS); }
                qmv_reg_rows<T, GS, BITS, false, (LW % get_pack_factor<BITS, 32>()) == 0>(wu, su, bu, act, LW, rows, lid, r);
            }
            if (lid < (uint)UPRPS && (int)((sg * (uint)UPRPS + lid) / (uint)COLS) < HC) {
                const int slot = (int)sg * UPRPS + (int)lid;
                T weight;
                if constexpr (UPRPS == 4) {
                    const float low = metal::select(r[0], r[1], (lid & 1u) != 0);
                    const float high = metal::select(r[2], r[3], (lid & 1u) != 0);
                    weight = static_cast<T>(metal::select(low, high, (lid & 2u) != 0));
                } else {
                    weight = static_cast<T>(metal::select(r[0], r[1], (lid & 1u) != 0));
                }
                const int row = d0 + (slot % COLS) + H * (slot / COLS);
        ...
        if (t < (uint)COLS) {
            ...
            for (int v = 0; v < VPT; ++v) {
                T acc = T(0);
                for (int s = 0; s < HC; ++s) {
                    const T p = products[s * COLS + (int)t][v];
                    acc = acc + p;
                }
    ...
    static func upMix(
        ...
        let rps = (S == 1 && packedRows) ? TrackFastMixerKernels.upRowsPerSimdgroup : 4
        let cols = rps / 2
        precondition(hidden % cols == 0)
        let outs = (S == 1 ? upMixKernel1 : upMixKernel)(
            ...
            template: [
                ...
                ("PACKED_ROWS", packedRows), ("UPRPS", rps),
            ],
            grid: (32, (hidden / cols) * 2, 1), threadGroup: (32, 2, 1),
