...
enum TrackFastMixerKernels {
    ...
    // ownership. At the served shape (KD = hcCount * hidden = 10240, ND = 320)
    // the launch is ND / (2 * RPS) + 2 tiles of 64 threads, so RPS picks between
    // 162, 82 and 42 threadgroups. MLXFAST-INJSPLIT moved that formula; re-swept
    // after it, one is the best of the three and two the runner-up, ordered the
    // way an underfilled launch would predict. Ownership only: each row keeps its
    // own K walk, so every setting is bit-exact with the others (asserted in the
    // local mixer bench, max|diff| == 0).
