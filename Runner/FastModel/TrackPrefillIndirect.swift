...
enum TrackPrefillIndirect {
    ...
    private static let kernel = MLXFast.metalKernel(
        ...
        inputNames: ["x", "w", "scales", "biases", "indices", "token_rows", "expert_bounds"],
        ...
        -> (activated: MLXArray, sortedIDs: MLXArray, inverse: MLXArray)?
    {
        ...
        func project(_ bank: (w: MLXArray, s: MLXArray, b: MLXArray)) -> MLXArray {
            kernel(
                [x, bank.w, bank.s, bank.b, sortedIDs, tokenRows, expertBounds ?? sortedIDs],
                template: [("T", x.dtype), ("M", rows), ("N", 640), ("K", 2560),
                           ("HAS_EXPERT_BOUNDS", expertBounds != nil)],
    ...
    static let source = #"""
        ...
        track_prefill_indirect<T, 32, 4, 32, 64, 64, 2, 2, true, HAS_EXPERT_BOUNDS>(
            x, w, scales, biases, indices, token_rows, y,
            M, N, K, Ws, As, threadgroup_position_in_grid,
            simdgroup_index_in_threadgroup, thread_index_in_simdgroup, expert_bounds);
        """#
}
