...
enum TrackPrefillIndirect {
        ...
        -> (activated: MLXArray, sortedIDs: MLXArray, inverse: MLXArray, tiles: MLXArray)?
    {
        ...
        let tiles: MLXArray
        let d = m.expertDown
        if d.w.shape == [512, 2560, 80], d.s.shape == [512, 2560, 20], d.b.shape == d.s.shape,
            d.w.dtype == .uint32, d.s.dtype == .bfloat16, d.b.dtype == .bfloat16,
            let c = TrackExpertPacking.apply(
            flatIDs: flatIDs, experts: experts, topK: indices.dim(2))
        {
            (sortedIDs, tokenRows, inverse, tiles) = (c.sortedIDs, c.tokenRows, c.inverse, c.tiles)
            ...
            flatIDs: flatIDs, experts: g.w.dim(0), topK: indices.dim(2))
        {
            ...
            tiles = tileTable(sortedIDs: sortedIDs, rows: rows, experts: experts)
        } else {
