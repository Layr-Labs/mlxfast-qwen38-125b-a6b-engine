...
enum TrackPrefillIndirect {
    ...
    ///
    /// `MLXFAST_GU_TILE` selects an accepted pair (`64x64`, `128x32`).
    /// Unset or unrecognized → `64x64`, the tip form. Rejected BN∈{64,128}
    /// × BK∈{32,64} candidates stay in `rejectedGUTiles`.
    ///
    ...
    static func gateUpSource(tile: GUTile) -> String {
        tile == defaultGUTile ? sourceGU : sourceGU128x32
    }
    ...
    private static let selectedGUTile =
        guTile(named: ProcessInfo.processInfo.environment["MLXFAST_GU_TILE"])
    ...
    private static let gateUpKernel128x32 = MLXFast.metalKernel(
        name: "track_prefill_indirect_gate_up_128x32",
        inputNames: ["x", "w0", "scales0", "biases0", "w1", "scales1", "biases1", "indices", "token_rows", "tiles"],
        outputNames: ["y"], source: sourceGU128x32.replacingOccurrences(of: "y0, y1, N, K", with: "y, y, N, K"),
        header: metalHeader, ensureRowContiguous: true)
    ...
    static func apply(
        ...
        let maxT = maxTiles(rows: rows, experts: experts)
        let gu = selectedGUTile
        let activated: MLXArray
        if gu == defaultGUTile {
            activated = gateUpKernel(
            ...
            activated = gateUp(
                tile: gu, x: x, w0: g.w, scales0: g.s, biases0: g.b,
                w1: u.w, scales1: u.s, biases1: u.b,
                sortedIDs: sortedIDs, tokenRows: tokenRows, tiles: tiles)
        }
    ...
    /// Launch one accepted gate|up tile. The default 64×64 path in `apply`
    ...
    static func gateUp(
        ...
        let compiled = tile == defaultGUTile ? gateUpKernel : gateUpKernel128x32
        return compiled(
