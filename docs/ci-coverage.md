...
| `lint` | `ubuntu-latest` | `tools/test-build-bench-worker.sh` — the combined worker/Metal build selects `track-bench-worker`, stages the editable Runner product, detects stale source/artifact hashes, and refuses incomplete or misdirected builds; compiler and Metal work are stubbed |
...
| `lint` | `ubuntu-latest` | `tools/test-setup-onboarding.sh` — plain-clone submodule initialization, parallel checkpoint downloads from tiny local fixtures, fresh transform output publication and rollback, and explicit prebuilt/build-only setup behavior, with a stub Swift toolchain |
...
| `lint` | `ubuntu-latest` | `tools/test-local-baseline.sh` — the public local entry point, root proxy, and facade select the public fixture and staged worker, preserve overrides, cooling and exit status, and isolate the run from inherited paired-scoring settings |
