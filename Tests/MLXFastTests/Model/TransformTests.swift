...
/// THE VERIFIER REFUSES A TREE THAT DOES NOT DECLARE ITS NORM CONVENTION, and
/// it refuses BEFORE it regenerates and byte-compares. The byte compare would
/// also catch a stale tree, but only as an opaque `config.json` mismatch, so
/// the ordering is what makes the refusal say which key is missing and why.
/// A tree transformed before the key existed reaches exactly this path.
@Test
func transformVerifierRefusesAQwen4ExpTreeWithoutTheNormConvention() throws {
    let fixture = try writeTransformFixture()
    _ = try SwiftTransform.run(
        TransformOptions(referencePath: fixture.reference.path, outputPath: fixture.output.path)
    )
    // Overwrite the emitted config with a qwen4Exp-shaped one that predates
    // the key. This also breaks the byte compare, which is the point: the
    // norm refusal must be the one that fires.
    try #"{"model_type":"qwen4_exp_text"}"#.write(
    ...
    #expect(message.contains("rms_norm_weight_offset"))
    #expect(message.contains("refused"))
}

/// A tree that DOES declare the pinned convention gets past the gate and is
/// then judged on its bytes like any other. This pins that the gate is a
/// requirement on one key, not a blanket refusal of the family.
@Test
func transformVerifierNormConventionGateAcceptsThePinnedValue() throws {
    let root = try temporaryDirectory()
    let configPath = root.appendingPathComponent("config.json")
    try #"{"model_type":"qwen4_exp_text","rms_norm_weight_offset":0}"#.write(
        to: configPath, atomically: true, encoding: .utf8)
    try SwiftTransform.validateTransformedNormConvention(runtimeConfigPath: configPath)

    // A family with no such knob passes through untouched.
    let foreign = root.appendingPathComponent("foreign.json")
    try #"{"model_type":"laguna"}"#.write(to: foreign, atomically: true, encoding: .utf8)
    try SwiftTransform.validateTransformedNormConvention(runtimeConfigPath: foreign)
}
