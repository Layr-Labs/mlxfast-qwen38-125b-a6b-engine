#!/usr/bin/env bash
# Bootstrap system tools and build the Swift-only Qwen 3.8 125B A6B harness.
set -euo pipefail

# REPOINTED 2026-08-27 onto the Qwen 3.8 125B A6B MLX 4-bit checkpoint, in
# lockstep with MLXFastConstants.referenceModelRepository / _Revision in
# Sources/MLXFastCore/Constants.swift, the artifact fixture support under
# Tests/MLXFastTests/Model/, and fixtures/qwen3_8_125b_a6b_track.json's
# target.upstream_model_id / target.upstream_revision -- four independent
# sources that agree byte-for-byte on this pin.
#
# This checkpoint is NOT our own conversion: it is fetched directly from the
# published MLX 4-bit release, which carries the native MTP head inside the
# same shards (docs/qwen38-125b-a6b-port-notes.md). The repository is PUBLIC, so the
# default fetch is ANONYMOUS: no Hugging Face token, no login, nothing to
# configure. MLXFAST_REFERENCE_AUTH_HEADER remains as an OPTIONAL fallback for
# the two cases that still need one -- a private mirror staged by an operator,
# or an anonymous fetch being rate-limited -- and is empty by default. Every
# variable here stays env-overridable so a locally staged copy can still be
# used without editing this file.
REFERENCE_MODEL_REPO="${MLXFAST_REFERENCE_MODEL_REPO:-Vontra/Qwen3.8-Flash-Next-MLX-4bit-MTP}"
REFERENCE_REVISION="${MLXFAST_REFERENCE_REVISION:-327c8a604de613b42f84ba5e6b796c0931e8aa3b}"
...
DEFAULT_REFERENCE_BASE_URL="https://huggingface.co/Vontra/Qwen3.8-Flash-Next-MLX-4bit-MTP/resolve/${REFERENCE_REVISION}"
...
REFERENCE_MANIFEST_PATH="${MLXFAST_REFERENCE_MANIFEST_PATH:-fixtures/reference_qwen3_8_125b_a6b_4bit.sha256}"
...
# 260 GiB covers the ~105.5 GiB pinned snapshot (113233030116 bytes across 32
# manifest records) plus headroom for the text-tower transform's output
# alongside it, at the same ~2.5x snapshot-to-floor ratio the retired Gemma
# default used (40 GiB floor for a 14.6 GiB snapshot).
REFERENCE_MIN_FREE_GIB="${MLXFAST_REFERENCE_MIN_FREE_GIB:-260}"
# The pinned checkpoint has 22 safetensors shards. Six parallel jobs keeps the
# link saturated without opening 22 streams at once. Override with
# MLXFAST_REFERENCE_DOWNLOAD_JOBS.
REFERENCE_DOWNLOAD_JOBS="${MLXFAST_REFERENCE_DOWNLOAD_JOBS:-6}"
...
# macmon is optional (it only powers the benchmarker's local GPU cool-down gate).
...
# the benchmarker's gate lookup already searches -- never through Homebrew, so a
...
# WHICH COMMAND THE READER ACTUALLY TYPED. Every line this script prints is
# prefixed with this label. The default is its own name, which is right for a
# direct ./setup.sh run. A wrapper that drives this downloader as a subprocess
# sets this to its own name, so the transcript names the command that produced
# it. Cosmetic only: nothing about what is downloaded, verified or written
# depends on it. FAIL CLOSED on a label that is not a plain command name, so a
# stray value cannot inject shell-looking text into every line of a setup
# transcript.
SETUP_LOG_LABEL="${MLXFAST_SETUP_LOG_LABEL:-setup.sh}"
if [[ ! "${SETUP_LOG_LABEL}" =~ ^[A-Za-z0-9._-]+$ ]]; then
  # Literal name on purpose: the value being rejected must not be the thing
  # that labels its own rejection.
...
Important environment variables:
  ...
  MLXFAST_SETUP_LOG_LABEL            Command name this run prefixes its output
                                     with. Default: ${SETUP_LOG_LABEL}. Set by
                                     a wrapper that drives this downloader as a
                                     subprocess so the transcript names the
                                     command the reader actually typed.
                                     Cosmetic; changes nothing that is fetched.
  ...
  MLXFAST_SKIP_SWIFT_BUILD=1         Reuse the Swift products from a previous
                                     ...
                                     anyway. Used by a wrapper whose delegated
                                     download would otherwise repeat
  ...
  MLXFAST_SKIP_MACMON_INSTALL=1      Do not install macmon (the GPU temperature
                                     reader the benchmarker's local cool-down gate
...
if [[ "$#" -gt 0 ]]; then
  case "$1" in
    -h|--help|help)
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: unknown argument '$1'" >&2
...
print_setup_summary() {
  ...
  cat <<EOF
${SETUP_LOG_LABEL}: setup complete elapsed=$(format_duration "${elapsed}")
${SETUP_LOG_LABEL}: summary
  trusted CLI: ${SWIFT_BIN}
  scored engine: ${BENCH_WORKER_BIN}
...
ensure_homebrew() {
  ...
  if [[ "${MLXFAST_SKIP_HOMEBREW_INSTALL:-0}" == "1" ]]; then
    echo "${SETUP_LOG_LABEL}: Homebrew is not installed and MLXFAST_SKIP_HOMEBREW_INSTALL=1" >&2
    return 1
  fi

  if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "${SETUP_LOG_LABEL}: automatic Homebrew installation is only supported on macOS" >&2
    return 1
  fi

  if ! command -v curl >/dev/null 2>&1; then
    echo "${SETUP_LOG_LABEL}: curl is required to install Homebrew" >&2
    return 1
  fi

  echo "${SETUP_LOG_LABEL}: Homebrew not found; installing Homebrew"
  NONINTERACTIVE=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"

  if ! load_homebrew_shellenv || ! command -v brew >/dev/null 2>&1; then
    echo "${SETUP_LOG_LABEL}: Homebrew installation finished, but brew is still not on PATH" >&2
    echo "${SETUP_LOG_LABEL}: open a new shell or run Homebrew's shellenv command, then retry" >&2
    return 1
  fi
}
...
find_cmake() {
  ...
  if [[ -n "${MLXFAST_CMAKE_BIN:-}" ]]; then
    ...
    echo "${SETUP_LOG_LABEL}: MLXFAST_CMAKE_BIN is set but not executable: ${MLXFAST_CMAKE_BIN}" >&2
    return 1
  fi
...
ensure_cmake() {
  ...
  if [[ "${MLXFAST_SKIP_CMAKE_INSTALL:-0}" == "1" ]]; then
    echo "${SETUP_LOG_LABEL}: cmake is not installed and MLXFAST_SKIP_CMAKE_INSTALL=1" >&2
    return 1
  fi

  if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "${SETUP_LOG_LABEL}: automatic cmake installation is only supported on macOS" >&2
    return 1
  fi

  ensure_homebrew
  echo "${SETUP_LOG_LABEL}: installing cmake with Homebrew"
  brew install cmake

  if ! find_cmake >/dev/null; then
    echo "${SETUP_LOG_LABEL}: cmake installation finished, but cmake was not found" >&2
    return 1
  fi
}

find_macmon() {
  # Same lookup order as the benchmarker's local cool-down gate: explicit
  ...
  if [[ -n "${MLXFAST_MACMON_BIN:-}" ]]; then
    ...
    echo "${SETUP_LOG_LABEL}: MLXFAST_MACMON_BIN is set but not executable: ${MLXFAST_MACMON_BIN}" >&2
    return 1
  fi
...
install_macmon_release() {
  ...
  # ~/bin -- a location both find_macmon here and the benchmarker's cool-gate
  ...
  if [[ "${actual_hash}" != "${MACMON_RELEASE_SHA256}" ]]; then
    echo "${SETUP_LOG_LABEL}: macmon release sha256 mismatch (expected ${MACMON_RELEASE_SHA256}, got ${actual_hash}); not installing" >&2
    rm -rf "${staging_dir}"
    return 1
  fi
...
ensure_macmon() {
  ...
  # temperature reader the benchmarker's local cool-down gate uses to mirror the
  # ranked runner's 40C thermal gate. It is NOT required to run the benchmark:
  # if it stays missing, the benchmarker warns and skips the gate, so a failed
  # install must never fail setup. The install stays on by default because
  # the benchmarker directs participants to rerun ./setup.sh to get macmon, but
  # it is a pinned ~/bin binary drop, never a Homebrew mutation.
  local macmon_path
  if macmon_path="$(find_macmon)"; then
    echo "${SETUP_LOG_LABEL}: macmon found at ${macmon_path} (GPU cool-down gate enabled for local benchmark modes)"
    return 0
  fi

  if [[ "${MLXFAST_SKIP_MACMON_INSTALL:-0}" != "1" && "$(uname -s)" == "Darwin" ]]; then
    echo "${SETUP_LOG_LABEL}: installing macmon v${MACMON_VERSION} to ${MACMON_INSTALL_DIR}/macmon (GPU temperature reader for the local benchmark cool-down gate)"
    install_macmon_release || true
    if macmon_path="$(find_macmon)"; then
      echo "${SETUP_LOG_LABEL}: macmon installed at ${macmon_path}"
      return 0
    fi
  fi

  echo "${SETUP_LOG_LABEL}: macmon not found; the benchmarker's local GPU cool-down thermal gate will be skipped (install macmon on PATH or in ~/bin, or set MLXFAST_MACMON_BIN, to enable it)" >&2
  return 0
}
...
ensure_swift_toolchain() {
  ...
  if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "${SETUP_LOG_LABEL}: this Swift harness targets macOS on Apple Silicon" >&2
    exit 1
  fi

  if [[ "$(uname -m)" != "arm64" ]]; then
    echo "${SETUP_LOG_LABEL}: this Swift harness requires Apple Silicon (arm64)" >&2
    exit 1
  fi

  if ! command -v swift >/dev/null 2>&1; then
    echo "${SETUP_LOG_LABEL}: swift was not found; install Xcode command line tools with xcode-select --install" >&2
    exit 1
  fi

  # A pre-staged, fingerprint-verified mlx.metallib removes the only need for
  ...
  if ! command -v xcodebuild >/dev/null 2>&1; then
    echo "${SETUP_LOG_LABEL}: xcodebuild was not found; install Xcode" >&2
    exit 1
  fi
  ...
  if ! xcodebuild_output="$(xcodebuild -version 2>&1)"; then
    ...
    case "${xcodebuild_output}" in
      *"command line tools instance"*|*"requires Xcode"*)
        cat >&2 <<EOF
${SETUP_LOG_LABEL}: full Xcode is required, but the active developer directory is the
...
EOF
      ...
      *[Ll]icense*)
        cat >&2 <<EOF
${SETUP_LOG_LABEL}: the Xcode license has not been accepted for the selected Xcode
...
EOF
      ...
      *)
        cat >&2 <<EOF
${SETUP_LOG_LABEL}: xcodebuild is installed but not usable (developer directory: ${developer_dir:-unknown}).
...
# M5 runner does not honor ad-hoc values: the benchd benchmarker's trusted build step
...
ensure_metal_toolchain() {
  ...
  if [[ "${MLXFAST_SKIP_METAL_TOOLCHAIN_INSTALL:-0}" == "1" ]]; then
    cat >&2 <<EOF
${SETUP_LOG_LABEL}: Xcode's Metal Toolchain is not installed and MLXFAST_SKIP_METAL_TOOLCHAIN_INSTALL=1.
...
EOF
  ...
  echo "${SETUP_LOG_LABEL}: installing Xcode Metal Toolchain"
  if ! xcodebuild -downloadComponent MetalToolchain; then
    cat >&2 <<EOF
${SETUP_LOG_LABEL}: failed to install Xcode's Metal Toolchain.
...
EOF
  ...
  if ! metal_compiler_is_available; then
    echo "${SETUP_LOG_LABEL}: Metal Toolchain installation finished, but xcrun still cannot execute metal" >&2
    return 1
  fi
}
...
ensure_reference_space() {
  ...
  if ! [[ "${REFERENCE_MIN_FREE_GIB}" =~ ^[0-9]+$ ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_MIN_FREE_GIB must be an integer" >&2
    return 1
  fi
  ...
  if [[ -z "${available_kib}" || "${available_kib}" -lt "${required_kib}" ]]; then
    cat >&2 <<EOF
${SETUP_LOG_LABEL}: not enough free disk space for ${REFERENCE_MODEL_REPO}.
...
download_url_for_file() {
  ...
  case "${REFERENCE_APPEND_DOWNLOAD_QUERY}" in
    ...
    auto|"")
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_APPEND_DOWNLOAD_QUERY must be auto, true, or false" >&2
      return 1
      ;;
  esac
...
reference_hash_verification_enabled() {
  case "${REFERENCE_HASH_VERIFY}" in
    ...
    1|true|TRUE|yes|YES)
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_HASH_VERIFY must be 0 or 1" >&2
      return 2
      ;;
  esac
}
...
reference_post_download_full_verify_enabled() {
  case "${REFERENCE_POST_DOWNLOAD_FULL_VERIFY}" in
    ...
    1|true|TRUE|yes|YES)
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_POST_DOWNLOAD_FULL_VERIFY must be 0 or 1" >&2
      return 2
      ;;
  esac
}
...
reference_file_is_current() {
  ...
  if [[ ! -f "${REFERENCE_MANIFEST_PATH}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest missing at ${REFERENCE_MANIFEST_PATH}" >&2
    return 1
  fi
  ...
  if ! manifest_entry="$(reference_manifest_entry "${relative_path}" "${manifest_snapshot}")"; then
    rm -f "${manifest_snapshot}"
    echo "${SETUP_LOG_LABEL}: reference manifest has no entry for ${relative_path}" >&2
    return 1
  fi
  ...
  if [[ "${actual_size}" != "${expected_size}" ]]; then
    echo "${SETUP_LOG_LABEL}: cached ${label} size mismatch: expected ${expected_size}, got ${actual_size}"
    rm -f "${manifest_snapshot}"
    return 1
  fi
  ...
  if [[ "${before_signature}" != "${after_signature}" \
    ...
    echo "${SETUP_LOG_LABEL}: cached ${label} changed while SHA256 was being verified" >&2
    rm -f "${manifest_snapshot}"
    return 1
  fi
  if [[ "${actual_hash}" != "${expected_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: cached ${label} sha256 mismatch"
    echo "${SETUP_LOG_LABEL}: expected ${expected_hash}"
    echo "${SETUP_LOG_LABEL}: actual   ${actual_hash}"
    rm -f "${manifest_snapshot}"
    return 1
  fi
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" != "${verified_manifest_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest changed while ${label} was being verified" >&2
    rm -f "${manifest_snapshot}"
    return 1
  fi
...
reference_manifest_totals() {
  ...
  if [[ ! -f "${source_manifest}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest missing at ${source_manifest}" >&2
    return 1
  fi
...
# artifacts whose metadata trees legitimately disagree. The target
# (fixtures/reference_qwen3_8_125b_a6b_4bit.sha256) pins README.md,
# chat_template.jinja, config.json, generation_config.json, the index,
# preprocessor_config.json (vision-tower preprocessor config; unused by the
# text-only transform but published and downloaded like everything else),
# merges.txt, vocab.json and tokenizer.json/tokenizer_config.json, plus the 22
# safetensors shards -- 32 records, NO .gitattributes, NO LICENSE, NO
# qwen-logo.png.
#
# A hard-coded list was wrong in both directions: it demanded
# .gitattributes the manifest does not pin (a fresh ./setup.sh died on
# "reference manifest has no entry for .gitattributes"), it silently dropped
# processor_config.json for the backbone or added it where the assistant has
# none, and it silently skipped generation_config.json, which both manifests
# DO pin and which the ranked workflow's verify_cache -- a strict inventory in
# BOTH directions -- then rejects the cache for missing. Deriving the list
# from the manifest makes "what is pinned" and "what is fetched" the same list
# by construction.
#
...
reference_manifest_metadata_files() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    read -r expected_hash expected_size relative_path extra <<< "${line}"
    if [[ -n "${extra:-}" || -z "${expected_hash:-}" || -z "${expected_size:-}" || -z "${relative_path:-}" ]]; then
      echo "${SETUP_LOG_LABEL}: malformed record in ${source_manifest}: ${line}" >&2
      return 1
    fi
  ...
  if [[ "${emitted}" -eq 0 ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest ${source_manifest} names no non-shard metadata files" >&2
    return 1
  fi
}
...
write_verified_reference_marker() {
  ...
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" != "${expected_manifest_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest changed before ${relative_path} marker publication" >&2
    return 1
  fi
  ...
  if [[ -z "${expected_content_identity}" \
    ...
    echo "${SETUP_LOG_LABEL}: ${relative_path} changed before its verification marker was written" >&2
    return 1
  fi
...
reference_cache_lock_is_current() {
  ...
  if [[ "${current_manifest_hash}" != "${expected_manifest_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest changed while the cache lock was being validated" >&2
    return 1
  fi

  echo "${SETUP_LOG_LABEL}: trusted reference cache lock at ${lock_path}; skipping full SHA256 verification"
  return 0
}
...
write_reference_cache_lock() {
  ...
  if [[ -z "${files_path}" || ! -f "${files_path}" \
    ...
    echo "${SETUP_LOG_LABEL}: refusing to stamp reference files without bound SHA256 verification" >&2
    return 1
  fi
  ...
  if [[ "${manifest_hash}" != "${REFERENCE_VERIFIED_MANIFEST_HASH}" ]]; then
    discard_reference_verified_signatures
    echo "${SETUP_LOG_LABEL}: reference manifest changed after SHA256 verification" >&2
    return 1
  fi
  ...
  if ! temp_path="$(mktemp "${lock_path}.tmp.XXXXXX")"; then
    discard_reference_verified_signatures
    echo "${SETUP_LOG_LABEL}: failed to create temporary reference cache stamp" >&2
    return 1
  fi
  ...
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" \
    ...
    rm -f "${temp_path}"
    discard_reference_verified_signatures
    echo "${SETUP_LOG_LABEL}: reference manifest changed before cache stamp publication" >&2
    return 1
  fi
  ...
  echo "${SETUP_LOG_LABEL}: wrote reference cache lock ${lock_path}"
}
...
validate_reference_download_settings() {
  ...
  for name in \
    ...
    if ! [[ "${value}" =~ ^[1-9][0-9]*$ ]]; then
      echo "${SETUP_LOG_LABEL}: ${name} must be a positive integer" >&2
      return 1
    fi
  done
}
...
download_reference_file() {
  ...
  if reference_file_is_current "${file}" "${output_path}" "${label}"; then
    echo "${SETUP_LOG_LABEL}: using cached ${label}"
    mark_reference_file_complete \
  ...
  for base_url in "${base_urls[@]}"; do
    ...
    if [[ "${source_index}" -gt 1 ]]; then
      echo "${SETUP_LOG_LABEL}: trying fallback source for ${label}: ${base_url}"
    fi

    attempt=1
    while [[ "${attempt}" -le 2 ]]; do
      if [[ "${attempt}" == "1" ]]; then
        echo "${SETUP_LOG_LABEL}: downloading ${label}"
      else
        echo "${SETUP_LOG_LABEL}: redownloading ${label} from scratch after hash verification failed"
        rm -f "${partial_path}" "${marker_path}" || return 1
      fi

      # --silent --show-error, matching fetch_shard_from_source below: the
      # "downloading ${label}" / "downloaded ${label} elapsed=" pair around this
      # transfer is this script's progress report. Without --silent curl adds
      # its own ANSI progress meter on stderr, which interleaved a block of raw
      # percent/speed columns into an otherwise line-oriented transcript --
      # visible on any run whose metadata files are not already cached, and
      # loudest on the ~32 MB tokenizer.json. --show-error keeps real curl
      # diagnostics, and stall detection is unchanged (--speed-limit /
      # --speed-time abort the transfer regardless of the meter).
      curl_status=0
      ...
      if [[ "${curl_status}" != "0" ]]; then
        echo "${SETUP_LOG_LABEL}: ${label} source failed or stalled (status=${curl_status}, source=${base_url})" >&2
        break
      fi
      ...
      if reference_file_is_current "${file}" "${partial_path}" "${label}"; then
        ...
        echo "${SETUP_LOG_LABEL}: downloaded ${label} elapsed=$(format_duration "$((SECONDS - started_seconds))")"
        return 0
      fi
  ...
  echo "${SETUP_LOG_LABEL}: failed to download verified ${label}" >&2
  return 1
}
...
start_reference_download_heartbeat() {
  ...
  (
    ...
    while :; do
      ...
      echo "${SETUP_LOG_LABEL}: still downloading safetensors shard(s): ${progress} elapsed=$(format_duration "${elapsed}")"
    done
  ) &
  REFERENCE_DOWNLOAD_HEARTBEAT_PID="$!"
}
...
download_reference_shards() {
  ...
  if ! [[ "${jobs}" =~ ^[1-9][0-9]*$ ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_DOWNLOAD_JOBS must be a positive integer" >&2
    return 1
  fi
  validate_reference_download_settings || return 1
  ...
  if [[ "${jobs}" == "1" || "$#" -le 1 ]]; then
    local file
    local ordinal=0
    echo "${SETUP_LOG_LABEL}: downloading ${total} safetensors shard(s) with 1 parallel job"
    for file in "$@"; do
      ordinal=$((ordinal + 1))
      download_reference_file "${file}" "${output_dir}/${file}" "shard ${ordinal}/${total}: ${file}" || return 1
    done
    echo "${SETUP_LOG_LABEL}: downloaded ${total}/${total} safetensors shard(s) elapsed=$(format_duration "$((SECONDS - started_seconds))")"
    return 0
  fi
  ...
  if [[ "${expected_total_bytes}" -gt 0 ]]; then
    echo "${SETUP_LOG_LABEL}: downloading ${total} safetensors shard(s) with ${jobs} parallel job(s) ($(awk -v total="${expected_total_bytes}" 'BEGIN { printf "%.1f", total / 1073741824 }') GiB total; progress prints every ${REFERENCE_DOWNLOAD_PROGRESS_SECONDS}s)"
  else
  ...
  for file in "$@"; do
    ...
    mark_complete() {
      ...
      if [[ "$(shasum -a 256 "${REFERENCE_MANIFEST_PATH}" | awk "{print \$1}")" \
        ...
        echo "${SETUP_LOG_LABEL}: reference manifest changed before ${file} marker publication" >&2
        return 1
      fi
      current_content_identity="$(file_content_identity_signature "${candidate_path}")"
      if [[ -z "${expected_content_identity}" \
          || "${current_content_identity}" != "${expected_content_identity}" ]]; then
        echo "${SETUP_LOG_LABEL}: ${file} changed before its verification marker was written" >&2
        return 1
      fi
    ...
    reference_file_is_current() {
      ...
      if [[ -z "${expected_hash}" || -z "${expected_size}" ]]; then
        echo "${SETUP_LOG_LABEL}: reference manifest has no entry for ${file}" >&2
        return 1
      fi
      ...
      if [[ "${actual_size}" != "${expected_size}" ]]; then
        echo "${SETUP_LOG_LABEL}: cached shard ${ordinal}/${total}: ${file} size mismatch: expected ${expected_size}, got ${actual_size}"
        return 1
      fi
      ...
      if [[ "${before_signature}" != "${after_signature}" \
          || "${before_content_identity}" != "${after_content_identity}" ]]; then
        echo "${SETUP_LOG_LABEL}: cached shard ${ordinal}/${total}: ${file} changed while SHA256 was being verified" >&2
        return 1
      fi
      if [[ "${actual_hash}" != "${expected_hash}" ]]; then
        echo "${SETUP_LOG_LABEL}: cached shard ${ordinal}/${total}: ${file} sha256 mismatch"
        echo "${SETUP_LOG_LABEL}: expected ${expected_hash}"
        echo "${SETUP_LOG_LABEL}: actual   ${actual_hash}"
        return 1
      fi
      if [[ "$(shasum -a 256 "${REFERENCE_MANIFEST_PATH}" | awk "{print \$1}")" \
          != "${REFERENCE_DOWNLOAD_MANIFEST_HASH}" ]]; then
        echo "${SETUP_LOG_LABEL}: reference manifest changed while shard ${ordinal}/${total}: ${file} was verified" >&2
        return 1
      fi
    ...
    if reference_file_is_current "${output_path}"; then
      echo "${SETUP_LOG_LABEL}: using cached shard ${ordinal}/${total}: ${file}"
    ...
    for base_url in "${base_urls[@]}"; do
      ...
      if [[ "${source_index}" -gt 1 ]]; then
        ...
        echo "${SETUP_LOG_LABEL}: trying fallback source for shard ${ordinal}/${total}: ${file}: ${base_url}"
      fi
      url="${base_url%/}/${file}"
      ...
      while [[ "${attempt}" -le 2 ]]; do
        if [[ "${attempt}" == "1" ]]; then
          echo "${SETUP_LOG_LABEL}: downloading shard ${ordinal}/${total}: ${file} source=${source_label}"
        else
          echo "${SETUP_LOG_LABEL}: redownloading shard ${ordinal}/${total}: ${file} from scratch after hash verification failed"
        ...
        if [[ "${curl_status}" != "0" ]]; then
          echo "${SETUP_LOG_LABEL}: shard ${ordinal}/${total} source failed or stalled (status=${curl_status}, source=${base_url})" >&2
        ...
        if reference_file_is_current "${partial_path}"; then
          ...
          echo "${SETUP_LOG_LABEL}: downloaded shard ${ordinal}/${total}: ${file} elapsed=$((SECONDS - started_seconds))s source=${source_label}"
          exit 0
        fi
    ...
    echo "${SETUP_LOG_LABEL}: failed to download verified shard ${ordinal}/${total}: ${file}" >&2
    exit 1
  ...
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" != "${download_manifest_hash}" ]]; then
    rm -f "${download_manifest_snapshot}"
    echo "${SETUP_LOG_LABEL}: reference manifest changed during parallel shard verification" >&2
    return 1
  fi
  rm -f "${download_manifest_snapshot}"
  echo "${SETUP_LOG_LABEL}: downloaded ${total}/${total} safetensors shard(s) elapsed=$(format_duration "$((SECONDS - started_seconds))")"
...
# The manifest's *.safetensors records -- the exact complement of
# reference_manifest_metadata_files, which skips them.
reference_manifest_shard_files() {
  local source_manifest="${1:-${REFERENCE_MANIFEST_PATH}}"
  local line expected_hash expected_size relative_path extra
...
# THE one place a checkpoint's shard list is resolved.
#
# MULTI-SHARD (this track's target): upstream publishes
# model.safetensors.index.json, the index names the shards, and it is the only
# thing that can -- the manifest pins them but nothing says which order or which
# tensors live where.
#
# SINGLE-SHARD: a one-file checkpoint has nothing to index, so upstream
# publishes no index for one. The callers used to hard-require the index
# regardless, so such a checkpoint died on "missing
# model.safetensors.index.json" for a file that does not exist upstream and
# never will. This track's target is multi-shard, so the single-shard path is
# not exercised here; it stays because the rule below derives it from the pin
# rather than from a per-checkpoint special case.
#
# The single-shard acceptance is NARROW and derives from the PIN, not from the
# directory: the manifest must name EXACTLY ONE .safetensors record, and that
# record is the shard list. A manifest naming two or more without an index is
# still a hard failure -- that is a real missing index, not a single-shard
# checkpoint -- and a manifest naming none is likewise refused. So this can
# never turn "the index failed to download" into "assume one shard".
resolve_reference_shard_list() {
  local reference_dir="$1"
  local source_manifest="${2:-${REFERENCE_MANIFEST_PATH}}"
  ...
  pinned_shards="$(reference_manifest_shard_files "${source_manifest}")" || {
    echo "${SETUP_LOG_LABEL}: checkpoint at ${reference_dir} has no model.safetensors.index.json and its manifest (${source_manifest}) could not be read" >&2
    return 1
  }
  shard_count="$(printf '%s' "${pinned_shards}" | grep -c . || true)"
  if [[ "${shard_count}" != "1" ]]; then
    echo "${SETUP_LOG_LABEL}: checkpoint at ${reference_dir} is missing model.safetensors.index.json and its manifest pins ${shard_count} safetensors file(s); the index is optional ONLY for a single-shard checkpoint" >&2
    return 1
  fi
  printf '%s\n' "${pinned_shards}"
}
...
verify_reference_weights() {
  ...
  if [[ ! -f "${reference_dir}/config.json" ]]; then
    echo "${SETUP_LOG_LABEL}: reference checkpoint is missing config.json at ${reference_dir}" >&2
    return 1
  fi
  # The index is REQUIRED for a multi-shard checkpoint and ABSENT by
  # construction for a single-shard one; resolve_reference_shard_list is the one
  # place that distinction is made.
  if ! shard_list="$(resolve_reference_shard_list "${reference_dir}")"; then
    return 1
  fi
  ...
  if [[ "${#shard_files[@]}" -eq 0 ]]; then
    echo "${SETUP_LOG_LABEL}: checkpoint index did not list any safetensors shards" >&2
    return 1
  fi

  for file in "${shard_files[@]}"; do
    if [[ ! -s "${reference_dir}/${file}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference checkpoint is missing shard ${file} at ${reference_dir}" >&2
      missing=1
    fi
  done
  if [[ "${missing}" != "0" ]]; then
  ...
  if reference_cache_lock_is_current "${reference_dir}"; then
    echo "${SETUP_LOG_LABEL}: verified reference checkpoint at ${reference_dir} (${#shard_files[@]} safetensors shard(s))"
    return 0
  fi
...
verify_reference_weights_after_verified_download() {
  ...
  if [[ "${hash_status}" != "0" ]]; then
    echo "${SETUP_LOG_LABEL}: cannot skip post-download full verification unless MLXFAST_REFERENCE_HASH_VERIFY=1" >&2
    return 1
  fi
  ...
  echo "${SETUP_LOG_LABEL}: verified reference checkpoint at ${reference_dir} (${#shard_files[@]} safetensors shard(s)); skipped second SHA256 pass after verified downloads"
}
...
verify_reference_manifest_sizes() {
  ...
  if ! create_reference_verified_manifest_snapshot; then
    echo "${SETUP_LOG_LABEL}: failed to snapshot reference manifest" >&2
    discard_reference_verified_signatures
    return 1
  fi
  verified_manifest_hash="${REFERENCE_VERIFIED_MANIFEST_HASH}"
  if ! create_reference_verified_signatures_path; then
    echo "${SETUP_LOG_LABEL}: failed to create verified download signature record" >&2
    discard_reference_verified_signatures
    return 1
  fi
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    read -r expected_hash expected_size relative_path extra <<< "${line}"
    if [[ -n "${extra:-}" || -z "${expected_hash:-}" || -z "${expected_size:-}" || -z "${relative_path:-}" ]]; then
      echo "${SETUP_LOG_LABEL}: malformed reference manifest line: ${line}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    if [[ ! "${expected_hash}" =~ ^[0-9a-f]{64}$ || ! "${expected_size}" =~ ^[0-9]+$ ]]; then
    ...
    if [[ "${relative_path}" == /* || "${relative_path}" == *\\* ]]; then
      echo "${SETUP_LOG_LABEL}: unsafe reference manifest path: ${relative_path}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    case "/${relative_path}/" in
    ...
    file_path="${reference_dir}/${relative_path}"
    if [[ ! -f "${file_path}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference checkpoint is missing manifest file ${relative_path}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    actual_size="$(wc -c < "${file_path}" | tr -d ' ')"
    if [[ "${actual_size}" != "${expected_size}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference file ${relative_path} size mismatch: expected ${expected_size}, got ${actual_size}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    marker_path="${file_path}.complete"
    if [[ ! -f "${marker_path}" ]]; then
      echo "${SETUP_LOG_LABEL}: verified download marker missing for ${relative_path}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    ...
    if [[ "${marker_version}" != "version=2" \
      ...
      echo "${SETUP_LOG_LABEL}: invalid verified download marker for ${relative_path}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    ...
    if [[ "${actual_signature}" != "${marker_signature}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference file ${relative_path} changed after download verification" >&2
      discard_reference_verified_signatures
      return 1
    fi
    printf '%s\t%s\t%s\n' \
  ...
  if [[ "${checked}" -eq 0 ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest contained no files: ${REFERENCE_MANIFEST_PATH}" >&2
    discard_reference_verified_signatures
    return 1
  fi
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" != "${verified_manifest_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest changed while download markers were aggregated" >&2
    discard_reference_verified_signatures
    return 1
  fi
  echo "${SETUP_LOG_LABEL}: verified ${checked} downloaded reference file signature(s)"
}
...
setup_parallel_metallib_enabled() {
  case "${SETUP_PARALLEL_METALLIB}" in
    ...
    1|true|TRUE|yes|YES)
      ...
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: MLXFAST_SETUP_PARALLEL_METALLIB must be 0 or 1" >&2
      return 2
      ;;
  esac
}
...
recover_stale_reference_cache_mutation_lock() {
  ...
  echo "${SETUP_LOG_LABEL}: recovered stale reference cache mutation lock ${lock_dir}"
  return 0
}
...
acquire_reference_cache_mutation_lock() {
  ...
  if ! [[ "${REFERENCE_CACHE_MUTATION_LOCK_TIMEOUT_SECONDS}" =~ ^(0|[1-9][0-9]*)$ ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_CACHE_MUTATION_LOCK_TIMEOUT_SECONDS must be a non-negative integer" >&2
    return 1
  fi
  if ! [[ "${REFERENCE_CACHE_MUTATION_LOCK_STALE_SECONDS}" =~ ^[1-9][0-9]*$ ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_REFERENCE_CACHE_MUTATION_LOCK_STALE_SECONDS must be a positive integer" >&2
    return 1
  fi
  ...
  while true; do
    owner_temp="$(mktemp "${lock_dir}.owner.XXXXXX")" || {
      echo "${SETUP_LOG_LABEL}: failed to create reference cache mutation owner record" >&2
      return 1
    }
    ...
    if ! printf 'token=%s pid=%s host=%s started_at=%s\n' \
      ...
      rm -f "${owner_temp}"
      REFERENCE_CACHE_MUTATION_OWNER_TEMP=""
      echo "${SETUP_LOG_LABEL}: failed to initialize reference cache mutation lock owner" >&2
      return 1
    fi
    initializing_dir_raw="$(mktemp -d \
      ...
      rm -f "${owner_temp}"
      REFERENCE_CACHE_MUTATION_OWNER_TEMP=""
      echo "${SETUP_LOG_LABEL}: failed to create reference cache mutation lock generation" >&2
      return 1
    }
    ...
    if [[ -e "${generation_dir_raw}" || -L "${generation_dir_raw}" ]] \
      ...
      rm -rf "${initializing_dir}"
      REFERENCE_CACHE_MUTATION_LOCK_INITIALIZING_DIR=""
      echo "${SETUP_LOG_LABEL}: failed to publish initialized reference cache lock generation" >&2
      return 1
    fi
    REFERENCE_CACHE_MUTATION_LOCK_INITIALIZING_DIR=""
    ...
    if [[ ! -e "${lock_dir}" && ! -L "${lock_dir}" ]] \
      ...
      if [[ -L "${lock_dir}" \
        ...
        echo "${SETUP_LOG_LABEL}: acquired reference cache mutation lock ${lock_dir}"
        return 0
      fi
    ...
    if [[ -e "${lock_dir}" && ! -d "${lock_dir}" && ! -L "${lock_dir}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference cache mutation lock path exists and is not a lock directory: ${lock_dir}" >&2
      return 1
    fi
    ...
    if [[ "${announced_wait}" == "0" ]]; then
      echo "${SETUP_LOG_LABEL}: waiting for reference cache mutation lock ${lock_dir}"
      announced_wait=1
    fi
    if (( SECONDS - started_seconds >= REFERENCE_CACHE_MUTATION_LOCK_TIMEOUT_SECONDS )); then
      echo "${SETUP_LOG_LABEL}: timed out waiting for reference cache mutation lock ${lock_dir}" >&2
      if [[ -f "${lock_dir}/owner" ]]; then
        echo "${SETUP_LOG_LABEL}: lock owner: $(cat "${lock_dir}/owner")" >&2
      fi
      return 1
    fi
    sleep 1
...
release_reference_cache_mutation_lock() {
  ...
  if [[ -z "${REFERENCE_CACHE_MUTATION_LOCK_TOKEN}" \
    ...
    echo "${SETUP_LOG_LABEL}: refusing to release reference cache mutation lock owned by another process: ${lock_dir}" >&2
    return 1
  fi
  ...
  if [[ -L "${lock_dir}" ]]; then
    ...
    if [[ -z "${REFERENCE_CACHE_MUTATION_LOCK_GENERATION_DIR}" \
      ...
      echo "${SETUP_LOG_LABEL}: refusing to release a replaced reference cache mutation lock: ${lock_dir}" >&2
      return 1
    fi
    ...
    if ! rm -rf "${REFERENCE_CACHE_MUTATION_LOCK_GENERATION_DIR}"; then
      echo "${SETUP_LOG_LABEL}: failed to remove released reference cache lock generation" >&2
      return 1
    fi
    REFERENCE_CACHE_MUTATION_LOCK_GENERATION_DIR=""
  else
    ...
    if ! rmdir "${lock_dir}"; then
      echo "${SETUP_LOG_LABEL}: failed to release reference cache mutation lock ${lock_dir}" >&2
      return 1
    fi
    REFERENCE_CACHE_MUTATION_LOCK_HELD=0
    REFERENCE_CACHE_MUTATION_LOCK_TOKEN=""
  fi
  echo "${SETUP_LOG_LABEL}: released reference cache mutation lock ${lock_dir}"
}
...
assert_frozen_dependency_graph() {
  ...
  for manifest in Package.swift Package.resolved; do
    ...
    if ! git diff --quiet HEAD -- "${manifest}" 2>/dev/null; then
      echo "${SETUP_LOG_LABEL}: ${manifest} differs from the committed state; the dependency graph is frozen by challenge policy" >&2
      echo "${SETUP_LOG_LABEL}: restore it (git checkout -- ${manifest}) and rerun" >&2
      return 1
    fi
  done
}
...
build_swift_harness() {
  ...
  # the whole SwiftPM build -- about 25 seconds of no-op relinking, after which
  # nothing about the products has changed.
  #
  # FAIL OPEN, deliberately: the knob is honoured only when BOTH products are
  # already present and executable. A run on a fresh clone has no binaries to
  # reuse, and refusing there would turn a wasteful rebuild into a broken
  # script -- so a missing product builds anyway and says
  ...
  if [[ "${MLXFAST_SKIP_SWIFT_BUILD:-0}" == "1" ]]; then
    if ensure_swift_harness_ready 2>/dev/null; then
      echo "${SETUP_LOG_LABEL}: MLXFAST_SKIP_SWIFT_BUILD=1 and both products are present; reusing ${SWIFT_BIN} and ${BENCH_WORKER_BIN}"
      return 0
    fi
  ...
  if [[ ! -x "${BENCH_WORKER_BIN}" ]]; then
    echo "${SETUP_LOG_LABEL}: scored engine missing at ${BENCH_WORKER_BIN}; build failed or MLXFAST_BENCH_WORKER_EXECUTABLE is wrong" >&2
    return 1
  fi
}
...
build_mlx_metallib() {
  if [[ "${MLXFAST_SKIP_MLX_METALLIB:-0}" == "1" ]]; then
    echo "${SETUP_LOG_LABEL}: skipping mlx.metallib build"
    return 0
  fi
  ensure_cmake || return 1
  ensure_metal_toolchain || return 1
  echo "${SETUP_LOG_LABEL}: building mlx.metallib for MLX Swift runtime"
...
start_mlx_metallib_build() {
  ...
  case "${METALLIB_BUILD_STATE}" in
    running|completed)
      ...
      ;;
    failed)
      echo "${SETUP_LOG_LABEL}: mlx.metallib build already failed" >&2
      return 1
      ;;
    not_started)
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: invalid mlx.metallib build state: ${METALLIB_BUILD_STATE}" >&2
      return 1
      ;;
  esac
  if setup_parallel_metallib_enabled; then
    ...
    set +m
    echo "${SETUP_LOG_LABEL}: mlx.metallib build running in background pid=${METALLIB_BUILD_PID}"
  else
    parallel_status="$?"
...
wait_for_mlx_metallib_build() {
  ...
  case "${METALLIB_BUILD_STATE}" in
    running)
      if [[ -z "${METALLIB_BUILD_PID}" ]]; then
        echo "${SETUP_LOG_LABEL}: mlx.metallib build is running without a background pid" >&2
        METALLIB_BUILD_STATE="failed"
        return 1
      fi
      echo "${SETUP_LOG_LABEL}: waiting for mlx.metallib build"
      if ! wait "${METALLIB_BUILD_PID}"; then
        METALLIB_BUILD_PID=""
        METALLIB_BUILD_PROCESS_GROUP=""
        METALLIB_BUILD_STATE="failed"
        echo "${SETUP_LOG_LABEL}: mlx.metallib build failed" >&2
        return 1
      fi
      METALLIB_BUILD_PID=""
      METALLIB_BUILD_PROCESS_GROUP=""
    ...
    failed)
      ...
      ;;
    not_started)
      echo "${SETUP_LOG_LABEL}: mlx.metallib build was not started" >&2
      return 1
      ;;
    *)
      echo "${SETUP_LOG_LABEL}: invalid mlx.metallib build state: ${METALLIB_BUILD_STATE}" >&2
  ...
  if [[ ! -f "${MLX_METALLIB}" ]]; then
    echo "${SETUP_LOG_LABEL}: mlx.metallib missing at ${MLX_METALLIB}" >&2
    return 1
  fi
}

stage_bench_worker_for_benchd() {
  # The engine builds into its isolated scratch root (.build-worker/release) with
  # mlx.metallib beside it, but benchd resolves the engine at .build/release and
  # loads mlx.metallib from that same directory. Copy the finished pair into the
  # benchd-resolved location so a run needs no manual metallib copy. Must run
  # AFTER build_swift_harness and wait_for_mlx_metallib_build so both pair members
  # exist. See tools/stage-bench-worker.sh for the isolation rationale.
  echo "${SETUP_LOG_LABEL}: staging the scored runtime worker + mlx.metallib for benchd"
  tools/stage-bench-worker.sh || return 1
}
...
verify_reference_manifest() {
  ...
  case "${REFERENCE_HASH_VERIFY}" in
    0|false|FALSE|no|NO)
      echo "${SETUP_LOG_LABEL}: skipping reference SHA256 verification"
      return 0
      ;;
    1|true|TRUE|yes|YES)
      ;;
  ...
  verified_manifest_hash="${REFERENCE_VERIFIED_MANIFEST_HASH}"
  if ! create_reference_verified_signatures_path; then
    echo "${SETUP_LOG_LABEL}: failed to create bound reference verification record" >&2
    discard_reference_verified_signatures
    return 1
  fi
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    if [[ "${before_signature}" != "${after_signature}" \
        || "${before_content_identity}" != "${after_content_identity}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference file ${relative_path} changed while SHA256 was being verified" >&2
      discard_reference_verified_signatures
      return 1
    fi
    if [[ "${actual_hash}" != "${expected_hash}" ]]; then
      echo "${SETUP_LOG_LABEL}: reference file ${relative_path} sha256 mismatch" >&2
      echo "${SETUP_LOG_LABEL}: expected ${expected_hash}" >&2
      echo "${SETUP_LOG_LABEL}: actual   ${actual_hash}" >&2
      discard_reference_verified_signatures
      return 1
    fi
    printf '%s\t%s\t%s\n' \
  ...
  if [[ "$(reference_manifest_hash 2>/dev/null || true)" != "${verified_manifest_hash}" ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest changed while SHA256 verification was running" >&2
    discard_reference_verified_signatures
    return 1
  fi
  echo "${SETUP_LOG_LABEL}: verified ${checked} reference file hash(es)"
}
...
ensure_reference_compat_link() {
  ...
  if [[ -L "${link_path}" ]]; then
    ...
    cat >&2 <<EOF
${SETUP_LOG_LABEL}: compatibility reference path exists and is not a symlink: ${link_path}
...
EOF
  ...
  echo "${SETUP_LOG_LABEL}: linked ${link_path} -> ${reference_dir}"
}
...
download_reference_weights_locked() {
  ...
  echo "${SETUP_LOG_LABEL}: downloading ${REFERENCE_MODEL_REPO}@${REFERENCE_REVISION}"
  echo "${SETUP_LOG_LABEL}: primary reference source ${REFERENCE_BASE_URL}"
  if [[ -n "${REFERENCE_FALLBACK_BASE_URL}" ]]; then
    echo "${SETUP_LOG_LABEL}: fallback reference source ${REFERENCE_FALLBACK_BASE_URL}"
  fi
  echo "${SETUP_LOG_LABEL}: reference cache path ${reference_dir}"
  ...
  if [[ "${#metadata_files[@]}" -eq 0 ]]; then
    echo "${SETUP_LOG_LABEL}: reference manifest named no metadata files to download: ${REFERENCE_MANIFEST_PATH}" >&2
    return 1
  fi

  echo "${SETUP_LOG_LABEL}: manifest names ${#metadata_files[@]} metadata file(s)"
  ...
  if [[ ! -f "${reference_dir}/config.json" ]]; then
    echo "${SETUP_LOG_LABEL}: downloaded checkpoint is missing config.json" >&2
    return 1
  fi
  # Single-shard checkpoints publish no index upstream, so the shard list comes
  # from the pin instead. See resolve_reference_shard_list.
  if ! shard_list="$(resolve_reference_shard_list "${reference_dir}")"; then
    return 1
  fi
  start_mlx_metallib_build || return 1
  ...
  echo "${SETUP_LOG_LABEL}: checkpoint index lists ${#shard_files[@]} safetensors shard(s)"
  ...
  echo "${SETUP_LOG_LABEL}: reference weights ready at ${reference_dir}"
}
...
download_reference_weights() {
  ...
  if [[ -e "${reference_dir}" && ! -d "${reference_dir}" ]]; then
    cat >&2 <<EOF
${SETUP_LOG_LABEL}: ${reference_dir} exists but is not a directory.
...
EOF
  ...
  if [[ -f "${reference_dir}/config.json" ]] && verify_reference_weights "${reference_dir}"; then
    echo "${SETUP_LOG_LABEL}: reference weights became ready while waiting at ${reference_dir}"
    ensure_reference_compat_link "${reference_dir}" || operation_status=$?
  else
    if [[ -f "${reference_dir}/config.json" ]]; then
      echo "${SETUP_LOG_LABEL}: reference cache at ${reference_dir} is incomplete or stale; repairing changed files"
    fi
...
check_yukon_cli() {
  ...
  if command -v yukon >/dev/null 2>&1; then
    echo "${SETUP_LOG_LABEL}: Yukon CLI found at $(command -v yukon)"
    return 0
  fi
  ...
  for candidate in "${HOME}/.local/bin/yukon" "${HOME}/bin/yukon"; do
    if [[ -x "${candidate}" ]]; then
      ...
      cat >&2 <<EOF
${SETUP_LOG_LABEL}: warning: the Yukon CLI is installed at
...
EOF
  ...
  echo "${SETUP_LOG_LABEL}: note: use the Yukon CLI ('yukon') for account and submission commands. It is not on PATH and is installed by the external Yukon installer, not by setup.sh. Build, transform, correctness, and local benchmarks work without it, but install it (and put its bin directory on PATH) before 'yukon submit' -- see README.md 'Submitting'."
  return 0
}
...
if [[ "${MLXFAST_SKIP_WEIGHTS_DOWNLOAD:-0}" == "1" || "${SKIP_MODEL_DOWNLOAD:-0}" == "1" ]]; then
  ...
  wait_for_mlx_metallib_build
  stage_bench_worker_for_benchd
  echo "${SETUP_LOG_LABEL}: skipping reference weight download"
  check_yukon_cli
