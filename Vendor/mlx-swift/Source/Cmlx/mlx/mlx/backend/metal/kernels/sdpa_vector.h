...
[[kernel]] void sdpa_vector_2pass_1(
    ...
    uint3 tpg [[threadgroups_per_grid]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  // Four query heads reuse each K/V load when gqa % 4 == 0.
  // Two query heads reuse each K/V load when gqa % 2 == 0.
  ...
  if constexpr (metal::is_same_v<T, bfloat16_t> && D == 256 && V == 256) {
    ...
    if (N >= 1024 && tptg.z == 1 && gqa % 4 == 0 && !has_mask && !has_sinks) {
      const uint quad = tidtg.y;
      if (quad >= gqa / 4) { return; }
      const uint kv = tid.x;
      const uint batch = tid.y;
      const uint block = tid.z;
      const uint kv_batch = batch * tpg.x + kv;
      const uint head0 = (batch * tpg.x + kv) * gqa + quad * 4;
