...
[[kernel]] void sdpa_vector_2pass_1(
    ...
    uint3 tpg [[threadgroups_per_grid]],
    uint simd_lid [[thread_index_in_simdgroup]]) {
  // Threadgroup storage lives at kernel scope. Instantiations that cannot take
  // the staged-tile lane reserve one vector, not the tile.
  constexpr int tile_vecs =
      (metal::is_same_v<T, bfloat16_t> && D == 256 && V == 256) ? 16 * 64 : 1;
  threadgroup metal::vec<T, 4> shared_k[tile_vecs];
  threadgroup metal::vec<T, 4> shared_v[tile_vecs];
