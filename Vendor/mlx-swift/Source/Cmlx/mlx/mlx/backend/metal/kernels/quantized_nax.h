...
struct PackedNAXGroup32 {
  ...
  void store(threadgroup T* dst) const thread {
    ...
    const float b = float(bias);
    float sc[2] = {s, s / 16.0f};
    // MLXFAST-WVEC2: the same per-value arithmetic, stored four values at a
    // time (dst is 8-byte aligned: padded rows and 32-element column offsets,
    // and the staging arrays below are declared alignas(16)).
...
// P17: expert-aligned row ownership inside the existing sorted-RHS dispatch.
// No descriptors, new matrix calls, changed grids, or cross-request state.
// A tile starting at L+j*BM belongs to the original grid row floor(start/BM),
// where L is its expert run's first row. The owner may write past its original
// BM-row window, but no other owner writes those rows. This both avoids repeat
// dequantization at arbitrary global BM boundaries and reduces partial SM work.
// The floating-point body retains the original BM/BN/BK/WM/WN and NAX tiles.

// Return the expert-aligned tile owned by this global BM-row window, or an
// empty [begin,end) interval when this run has no tile start in the window.
// `offset` and `offset_next` are the ORIGINAL kernel's local run boundaries.
// These searches and the ownership predicate are threadgroup-uniform.
template <int BM>
...
template <
    ...
    int WN,
    bool transpose>
METAL_FUNC void p17_affine_gather_qmm_rhs_nax(
    const device T* x,
    const device uint32_t* w,
    const device T* scales,
    const device T* biases,
